import { _ as _export_sfc, a as __nuxt_component_0, u as useHead } from "../server.mjs";
import HeaderOne from "./HeaderOne-CQq1TBjR.js";
import { S as ShopFeatures } from "./ShopFeatures-CN3dWQQq.js";
import { F as FreelancerAwards } from "./FreelancerAwards-Bv05_C13.js";
import { T as TeamBreadcrumb } from "./CommonBreadcrumb-NhK14ET7.js";
import { i as icon_1, a as icon_2, b as icon_3, S as StartUpCounter } from "./features-icon-3-DnFTuLyM.js";
import { H as HomeServices } from "./HomeServices-Bj4bslBR.js";
import { _ as _imports_0, P as PersonalPortServices } from "./PersonalPortServices-Dr9wNU-b.js";
import { mergeProps, withCtx, createVNode, createTextVNode, useSSRContext, resolveComponent, toDisplayString, openBlock, createBlock } from "vue";
import { ssrRenderAttrs, ssrRenderAttr, ssrRenderComponent, ssrRenderList, ssrRenderClass, ssrInterpolate, ssrRenderStyle } from "vue/server-renderer";
import { S as SalScrollAnimationMixin } from "./SalScrollAnimationMixin-DP9bLDx2.js";
import { _ as _imports_0$1, a as _imports_1, b as _imports_2 } from "./contact-icon-2-Ci3HWRNX.js";
import { a as _imports_0$2, _ as _imports_1$1, b as _imports_2$1 } from "./contact-location-3-CJJ7DDnC.js";
import { A as ArchSingleService, s as ser_icon_1, a as ser_icon_2, b as ser_icon_3, c as ser_icon_4 } from "./ArchSingleService-CIVTJwOT.js";
import { a as award_icon_1, b as award_icon_2, c as award_icon_3 } from "./award-3-DFy9LnO-.js";
import { b as bg_law_3, a as bg_law_4, c as bg_law_5 } from "./services-5-B55th-jS.js";
import { C as CreativeSingleFeature } from "./CreativeSingleFeature-CQJoi3yw.js";
import { p as polity_ser_icon_1, a as polity_ser_icon_2, b as polity_ser_icon_3 } from "./features-3-D3-6gktW.js";
import BackToTop from "./BackToTop-Dzk5hrJh.js";
import Footer from "./Footer-85SZoxrw.js";
import "ofetch";
import "#internal/nuxt/paths";
import "hookable";
import "unctx";
import "h3";
import "unhead";
import "@unhead/shared";
import "vue-router";
import "radix3";
import "defu";
import "ufo";
import "@vue/devtools-api";
import "destr";
import "klona";
import "devalue";
import "toastify-js";
import "@yeger/vue-masonry-wall";
import "dayjs";
import "dayjs/locale/th.js";
import "vue-select";
import "dayjs/plugin/buddhistEra.js";
import "cookie-es";
import "ohash";
import "./HeaderInfo-Cgp-mq39.js";
import "./Language-DjIvJcaS.js";
import "./SearchPopup-CwNKVdMc.js";
import "./logo-MlJCTsTl.js";
import "./utils-Cinv2m6T.js";
import "./OffCanvasMain-B2oGt6VK.js";
import "./Social-Cx28hJNr.js";
import "./MobileMenus-CVq7gvQU.js";
import "./OffCanvasSix-UeRQiqg5.js";
import "./logo-black-DNxRSU5J.js";
import "sal.js";
import "./auth-Ds3h1m1J.js";
import "./fetch-DNDFQiYI.js";
import "./ImagePopup-DYm5hHIZ.js";
import "vue-easy-lightbox";
const _sfc_main$i = {};
function _sfc_ssrRender$i(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_nuxt_link = __nuxt_component_0;
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "services__area pt-110 pb-90" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Box Shadow Style</h3></div></div></div><div class="row"><div class="col-xxl-4 col-xl-4 col-lg-4 col-md-6"><div class="services__item-9 services__item-style-2 mb-30 transition-3"><div class="services__item-9-top d-flex align-items-start justify-content-between"><div class="services__icon-9"><span><svg width="47" height="42" viewBox="0 0 47 42" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.06035 29.4262L12.5801 37.0285C17.8613 42.3677 19.6652 42.2798 24.8812 37.0285L36.9867 24.79C41.203 20.5275 42.2679 17.6931 36.9867 12.3539L29.467 4.75152C23.838 -0.939241 21.3821 0.488942 17.1659 4.75152L5.06035 16.99C-0.133935 22.2633 -0.568603 23.7354 5.06035 29.4262Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M38.5975 32.1286L37.1631 34.5236C35.1419 37.9292 36.7067 40.7197 40.6405 40.7197C44.5742 40.7197 46.139 37.9292 44.1178 34.5236L42.6834 32.1286C41.5533 30.239 39.7059 30.239 38.5975 32.1286Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M1.21484 22.1312C13.2986 18.8134 26.0344 18.7036 38.1616 21.8456L39.2483 22.1312" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg><img${ssrRenderAttr("src", _imports_0)} alt=""></span></div><div class="services__btn-9">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<i class="fa-light fa-arrow-up-right"${_scopeId}></i>`);
      } else {
        return [
          createVNode("i", { class: "fa-light fa-arrow-up-right" })
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div></div><div class="services__content-9"><span class="services-project">86 Project</span><h3 class="services__title-9">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`Website design`);
      } else {
        return [
          createTextVNode("Website design")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</h3></div></div></div><div class="col-xxl-4 col-xl-4 col-lg-4 col-md-6"><div class="services__item-9 mb-30 services__item-style-2 transition-3"><div class="services__item-9-top d-flex align-items-start justify-content-between"><div class="services__icon-9"><span><svg width="32" height="40" viewBox="0 0 32 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M31 10.5V29.5C31 37.1 29.125 39 21.625 39H10.375C2.875 39 1 37.1 1 29.5V10.5C1 2.9 2.875 1 10.375 1H21.625C29.125 1 31 2.9 31 10.5Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M19.7637 7.65088H12.2637" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M15.9941 33.4891C17.5992 33.4891 18.9004 32.1706 18.9004 30.5441C18.9004 28.9176 17.5992 27.5991 15.9941 27.5991C14.3891 27.5991 13.0879 28.9176 13.0879 30.5441C13.0879 32.1706 14.3891 33.4891 15.9941 33.4891Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg><img${ssrRenderAttr("src", _imports_0)} alt=""></span></div><div class="services__btn-9">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<i class="fa-light fa-arrow-up-right"${_scopeId}></i>`);
      } else {
        return [
          createVNode("i", { class: "fa-light fa-arrow-up-right" })
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div></div><div class="services__content-9"><span class="services-project">63 Project</span><h3 class="services__title-9">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`Mobile App Design`);
      } else {
        return [
          createTextVNode("Mobile App Design")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</h3></div></div></div><div class="col-xxl-4 col-xl-4 col-lg-4 col-md-6"><div class="services__item-9 mb-30 services__item-style-2 transition-3"><div class="services__item-9-top d-flex align-items-start justify-content-between"><div class="services__icon-9"><span><svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" d="M33.2859 12.4C36.434 12.4 38.9859 9.84803 38.9859 6.7C38.9859 3.55198 36.434 1 33.2859 1C30.1379 1 27.5859 3.55198 27.5859 6.7C27.5859 9.84803 30.1379 12.4 33.2859 12.4Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M10.5 21.9009H20" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M10.5 29.5H27.6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M23.8 1H14.3C4.8 1 1 4.8 1 14.3V25.7C1 35.2 4.8 39 14.3 39H25.7C35.2 39 39 35.2 39 25.7V16.2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg><img${ssrRenderAttr("src", _imports_0)} alt=""></span></div><div class="services__btn-9">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<i class="fa-light fa-arrow-up-right"${_scopeId}></i>`);
      } else {
        return [
          createVNode("i", { class: "fa-light fa-arrow-up-right" })
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div></div><div class="services__content-9"><span class="services-project">75 Project</span><h3 class="services__title-9">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`Dashboard`);
      } else {
        return [
          createTextVNode("Dashboard")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</h3></div></div></div></div></div></section>`);
}
const _sfc_setup$i = _sfc_main$i.setup;
_sfc_main$i.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/BoxShadowStyle.vue");
  return _sfc_setup$i ? _sfc_setup$i(props, ctx) : void 0;
};
const BoxShadowStyle = /* @__PURE__ */ _export_sfc(_sfc_main$i, [["ssrRender", _sfc_ssrRender$i]]);
const _sfc_main$h = {
  mixins: [SalScrollAnimationMixin]
};
function _sfc_ssrRender$h(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "features__area pt-110 pb-120" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Box Shadow with Border</h3></div></div></div><div class="features__wrapper-15 white-bg"><div class="row gx-0"><div class="col-xl-4 col-lg-4 col-md-6"><div class="features__item-15 text-center" data-sal="slide-up" data-sal-delay="130" data-sal-duration="1000"><div class="features__icon-15 mb-40"><span><svg width="50" height="45" viewBox="0 0 50 45" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.40418 31.3194L13.5607 39.4281C19.2892 45.1229 21.2458 45.0292 26.9035 39.4281L40.0341 26.3745C44.6074 21.8281 45.7626 18.8049 40.0341 13.1101L31.8776 5.00139C25.772 -1.0684 23.1081 0.454904 18.5348 5.00139L5.40418 18.0549C-0.229959 23.6795 -0.701435 25.2496 5.40418 31.3194Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M41.7841 34.2018L40.2282 36.7562C38.0358 40.3887 39.7331 43.365 44 43.365C48.2669 43.365 49.9642 40.3887 47.7718 36.7562L46.2159 34.2018C44.9901 32.1863 42.9863 32.1863 41.7841 34.2018Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M1.23438 23.5386C14.3414 19.9999 28.1557 19.8827 41.3099 23.234L42.4886 23.5386" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="features__content-15"><h3 class="features__title-15">Build your shop</h3><p>Build a website that loads fast for the highest user experience.</p></div></div></div><div class="col-xl-4 col-lg-4 col-md-6"><div class="features__item-15 text-center" data-sal="slide-up" data-sal-delay="150" data-sal-duration="1000"><div class="features__icon-15 mb-40"><span><svg width="46" height="39" viewBox="0 0 46 39" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M24.1672 38.0499H10.2625C3.31012 38.0499 1 33.4297 1 28.7874V10.2625C1 3.31012 3.31012 1 10.2625 1H24.1672C31.1196 1 33.4297 3.31012 33.4297 10.2625V28.7874C33.4297 35.7398 31.0975 38.0499 24.1672 38.0499Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M39.5421 30.7456L33.4258 26.4554V12.5727L39.5421 8.28245C42.5343 6.19234 44.9984 7.46841 44.9984 11.1426V27.9075C44.9984 31.5817 42.5343 32.8577 39.5421 30.7456Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M21.8939 17.3248C23.7166 17.3248 25.1941 15.8473 25.1941 14.0247C25.1941 12.202 23.7166 10.7245 21.8939 10.7245C20.0713 10.7245 18.5938 12.202 18.5938 14.0247C18.5938 15.8473 20.0713 17.3248 21.8939 17.3248Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="features__content-15"><h3 class="features__title-15">Video Tutorials</h3><p>Build a website that loads fast for the highest user experience.</p></div></div></div><div class="col-xl-4 col-lg-4 col-md-6"><div class="features__item-15 text-center" data-sal="slide-up" data-sal-delay="170" data-sal-duration="1000"><div class="features__icon-15 mb-40"><span><svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M34.9844 13C38.2981 13 40.9844 10.3137 40.9844 7C40.9844 3.68629 38.2981 1 34.9844 1C31.6707 1 28.9844 3.68629 28.9844 7C28.9844 10.3137 31.6707 13 34.9844 13Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M11 23.001H21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M11 31H29" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M25 1H15C5 1 1 5 1 15V27C1 37 5 41 15 41H27C37 41 41 37 41 27V17" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="features__content-15"><h3 class="features__title-15">Speed Optimized</h3><p>Build a website that loads fast for the highest user experience.</p></div></div></div></div></div></div></section>`);
}
const _sfc_setup$h = _sfc_main$h.setup;
_sfc_main$h.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/BoxShadowWithBorder.vue");
  return _sfc_setup$h ? _sfc_setup$h(props, ctx) : void 0;
};
const BoxShadowWithBorder = /* @__PURE__ */ _export_sfc(_sfc_main$h, [["ssrRender", _sfc_ssrRender$h]]);
const _sfc_main$g = {};
function _sfc_ssrRender$g(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "contact__item-area pt-120 pb-90" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Contact Icon Box</h3></div></div></div><div class="row"><div class="col-xl-4 col-lg-4 col-md-6"><div class="contact__item text-center mb-30 transition-3 white-bg"><div class="contact__icon"><img${ssrRenderAttr("src", _imports_0$1)} alt=""></div><div class="contact__content"><span class="contact-item-subtitle">Contact</span><p><a href="mailto:location@website.com">location@website.com</a></p><p><a href="tel:602-762-472-96">+(602) 762 472 96</a></p></div></div></div><div class="col-xl-4 col-lg-4 col-md-6"><div class="contact__item text-center mb-30 transition-3 white-bg"><div class="contact__icon"><img${ssrRenderAttr("src", _imports_1)} alt=""></div><div class="contact__content"><span class="contact-item-subtitle">Location</span><p><a href="https://www.google.com/maps/place/102+Thompson+St,+New+York,+NY+10012,+USA/@40.7255437,-74.004061,17z/data=!3m1!4b1!4m5!3m4!1s0x89c2598c338f691d:0x6d5f394e7a1962cc!8m2!3d40.7255397!4d-74.0018723" target="_blank">88 New South Head Rd, Triple, New York</a></p></div></div></div><div class="col-xl-4 col-lg-4 col-md-6"><div class="contact__item text-center mb-30 transition-3 white-bg"><div class="contact__icon"><img${ssrRenderAttr("src", _imports_2)} alt=""></div><div class="contact__content"><span class="contact-item-subtitle">Social Media</span><p>Follow on social media</p><div class="contact__social"><a href="#"><i class="fa-brands fa-facebook-f"></i></a><a href="#"><i class="fa-brands fa-twitter"></i></a><a href="#"><i class="fa-brands fa-linkedin-in"></i></a></div></div></div></div></div></div></div>`);
}
const _sfc_setup$g = _sfc_main$g.setup;
_sfc_main$g.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ContactIconBox.vue");
  return _sfc_setup$g ? _sfc_setup$g(props, ctx) : void 0;
};
const ContactIconBox = /* @__PURE__ */ _export_sfc(_sfc_main$g, [["ssrRender", _sfc_ssrRender$g]]);
const _sfc_main$f = {};
function _sfc_ssrRender$f(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "contact__loacation-box-area pt-110 pb-90" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Contact Icon Box 2</h3></div></div></div><div class="row"><div class="col-lg-4 col-md-6"><div class="contact__location-box text-center white-bg mb-30"><div class="contact__location-box-icon"><img${ssrRenderAttr("src", _imports_0$2)} alt=""></div><div class="contact__location-box-content"><h3 class="contact__location-box-title">Sany Francisco</h3><div class="contact__location-box-info"><p><a href="mailto:location@website.com">location@website.com</a></p><p><a href="tel:686-324-6838">686-324-6838</a></p></div><div class="contact__location-box-btn"><a href="https://www.google.com/maps/place/102+Thompson+St,+New+York,+NY+10012,+USA/@40.7255437,-74.004061,17z/data=!3m1!4b1!4m5!3m4!1s0x89c2598c338f691d:0x6d5f394e7a1962cc!8m2!3d40.7255397!4d-74.0018723" target="_blank" class="tp-btn-border">View Location</a></div></div></div></div><div class="col-lg-4 col-md-6"><div class="contact__location-box text-center white-bg mb-30"><div class="contact__location-box-icon"><img${ssrRenderAttr("src", _imports_1$1)} alt=""></div><div class="contact__location-box-content"><h3 class="contact__location-box-title">Paris</h3><div class="contact__location-box-info"><p><a href="mailto:location@website.com">location@website.com</a></p><p><a href="tel:686-324-6838">686-324-6838</a></p></div><div class="contact__location-box-btn"><a href="https://www.google.com/maps/place/102+Thompson+St,+New+York,+NY+10012,+USA/@40.7255437,-74.004061,17z/data=!3m1!4b1!4m5!3m4!1s0x89c2598c338f691d:0x6d5f394e7a1962cc!8m2!3d40.7255397!4d-74.0018723" target="_blank" class="tp-btn-border">View Location</a></div></div></div></div><div class="col-lg-4 col-md-6"><div class="contact__location-box text-center white-bg mb-30"><div class="contact__location-box-icon"><img${ssrRenderAttr("src", _imports_2$1)} alt=""></div><div class="contact__location-box-content"><h3 class="contact__location-box-title">Egpyt</h3><div class="contact__location-box-info"><p><a href="mailto:location@website.com">location@website.com</a></p><p><a href="tel:686-324-6838">686-324-6838</a></p></div><div class="contact__location-box-btn"><a href="https://www.google.com/maps/place/102+Thompson+St,+New+York,+NY+10012,+USA/@40.7255437,-74.004061,17z/data=!3m1!4b1!4m5!3m4!1s0x89c2598c338f691d:0x6d5f394e7a1962cc!8m2!3d40.7255397!4d-74.0018723" target="_blank" class="tp-btn-border">View Location</a></div></div></div></div></div></div></section>`);
}
const _sfc_setup$f = _sfc_main$f.setup;
_sfc_main$f.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ContactIconBoxTwo.vue");
  return _sfc_setup$f ? _sfc_setup$f(props, ctx) : void 0;
};
const ContactIconBoxTwo = /* @__PURE__ */ _export_sfc(_sfc_main$f, [["ssrRender", _sfc_ssrRender$f]]);
const _sfc_main$e = {
  components: { ArchSingleService },
  data() {
    return {
      service_data: [
        {
          icon: ser_icon_1,
          title: "Planning",
          subtitle: "There are many variations of passages of lorem Ipsum available.!",
          project: "350"
        },
        {
          icon: ser_icon_2,
          title: "Architecture",
          subtitle: "There are many variations of passages of lorem Ipsum available.!",
          project: "120"
        },
        {
          icon: ser_icon_3,
          title: "Design",
          subtitle: "There are many variations of passages of lorem Ipsum available.!",
          project: "240"
        },
        {
          icon: ser_icon_4,
          title: "Products",
          subtitle: "There are many variations of passages of lorem Ipsum available.!",
          project: "220"
        }
      ]
    };
  }
};
function _sfc_ssrRender$e(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_arch_single_service = resolveComponent("arch-single-service");
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "services__area pt-115 pb-90 black-bg-12" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="section__title-wrapper-8 text-center mb-60"><span class="section__title-pre-8">Animated Border</span><h3 class="section__title-8"><span class="section__title-highlight-8">Animated <svg width="206" height="24" viewBox="0 0 206 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path class="wow" d="M3 20.7505C47.375 13.8755 135.812 -15.4995 203 20.7505" stroke="#FF5A1B" stroke-width="6" stroke-linecap="round"></path></svg></span> Border Style </h3></div></div></div><div class="row"><!--[-->`);
  ssrRenderList($data.service_data, (item, i) => {
    _push(`<div class="col-lg-6 col-md-6">`);
    _push(ssrRenderComponent(_component_arch_single_service, { item }, null, _parent));
    _push(`</div>`);
  });
  _push(`<!--]--></div></div></section>`);
}
const _sfc_setup$e = _sfc_main$e.setup;
_sfc_main$e.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElAnimatedBorderStyle.vue");
  return _sfc_setup$e ? _sfc_setup$e(props, ctx) : void 0;
};
const ElAnimatedBorderStyle = /* @__PURE__ */ _export_sfc(_sfc_main$e, [["ssrRender", _sfc_ssrRender$e]]);
const _sfc_main$d = {
  mixins: [SalScrollAnimationMixin],
  data() {
    return {
      award_data: [
        {
          delay: "150",
          icon: award_icon_1,
          title: "2 x Pencils",
          subtitle: "Best Branded Editorial Experience",
          bg: ""
        },
        {
          delay: "170",
          icon: award_icon_2,
          title: "Creative Review Annual",
          subtitle: "Best Branded Editorial Experience",
          bg: "award__brown"
        },
        {
          delay: "190",
          icon: award_icon_3,
          title: "Webby Award",
          subtitle: "Best Branded Editorial Experience",
          bg: "award__purple"
        }
      ]
    };
  }
};
function _sfc_ssrRender$d(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "award__area pt-110 pb-90 p-relative z-index-1" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Background Color Box</h3></div></div></div><div class="row gx-1"><!--[-->`);
  ssrRenderList($data.award_data, (item, i) => {
    _push(`<div class="col-lg-4 col-md-6"><div class="${ssrRenderClass(`award__item-5 ${item.bg} mb-30`)}" data-sal="slide-up"${ssrRenderAttr("data-sal-delay", item.delay)} data-sal-duration="1000"><div class="award__icon-5"><img${ssrRenderAttr("src", item.icon)} alt=""></div><div class="award__content-5"><h4 class="award__title-5"><a href="#">${ssrInterpolate(item.title)}</a></h4><p>${ssrInterpolate(item.subtitle)}</p></div></div></div>`);
  });
  _push(`<!--]--></div></div></section>`);
}
const _sfc_setup$d = _sfc_main$d.setup;
_sfc_main$d.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElBgClrBox.vue");
  return _sfc_setup$d ? _sfc_setup$d(props, ctx) : void 0;
};
const ElBgClrBox = /* @__PURE__ */ _export_sfc(_sfc_main$d, [["ssrRender", _sfc_ssrRender$d]]);
const _sfc_main$c = {
  mixins: [SalScrollAnimationMixin],
  data() {
    return {
      services_data: [
        {
          id: 1,
          delay: ".7s",
          bg: bg_law_3,
          icon: `<svg width="62" height="60" viewBox="0 0 62 60" fill="none">
                  <path
                    d="M39.4183 35.377L39.0769 35.7183C38.5728 36.2223 38.2956 36.8942 38.2956 37.6102C38.2956 38.326 38.5728 38.998 39.0769 39.5019L41.7142 42.1392C42.2357 42.6607 42.9209 42.9218 43.6061 42.9218C44.2913 42.9218 44.9765 42.661 45.498 42.1394L58.2788 29.3582C59.3213 28.3151 59.3213 26.618 58.2788 25.5752L55.641 22.9376C55.1375 22.4337 54.4654 22.1561 53.7491 22.1561C53.0328 22.1561 52.3614 22.4335 51.8577 22.9374L51.5164 23.2788C43.6395 17.6715 42.3886 16.4208 36.7808 8.54395L37.1222 8.20258C37.6264 7.69868 37.9035 7.02672 37.9035 6.31071C37.9035 5.59469 37.6264 4.92262 37.1222 4.41883L34.4849 1.78152C33.9808 1.27762 33.3093 1 32.593 1C31.8768 1 31.2053 1.27762 30.7012 1.78152L17.9208 14.562C17.4166 15.0659 17.1395 15.7379 17.1395 16.4539C17.1395 17.1699 17.4166 17.842 17.9208 18.3458L20.5581 20.9835C21.0796 21.5048 21.7648 21.7655 22.45 21.7655C23.1352 21.7655 23.8204 21.5048 24.3419 20.9832L24.6832 20.6418C26.8174 22.1613 28.4691 23.3631 29.8481 24.4606L2.01808 50.9872C1.40155 51.575 1.05152 52.322 1.00511 53.1478C0.957648 54.0011 1.24159 54.761 1.82601 55.3453L4.71503 58.2338C5.25714 58.7768 5.95078 59.0602 6.73101 59.0602C6.79125 59.0602 6.85136 59.0586 6.91265 59.0552C7.73824 59.0088 8.48519 58.6585 9.07324 58.0416L35.5991 30.2118C36.6966 31.5909 37.8987 33.2425 39.4183 35.377ZM53.7491 23.7779C54.032 23.7779 54.2965 23.8866 54.4945 24.0843L57.1322 26.7219C57.5429 27.1326 57.5429 27.801 57.1322 28.212L44.3514 40.9926C43.9412 41.4034 43.2719 41.4038 42.8607 40.9926L40.2233 38.3553C39.8126 37.9443 39.8126 37.2758 40.2233 36.8649L53.0043 24.0841C53.2016 23.8866 53.4661 23.7779 53.7491 23.7779ZM21.7045 19.8369L19.0672 17.1993C18.8698 17.0016 18.7606 16.7369 18.7606 16.454C18.7606 16.1711 18.8698 15.9066 19.0672 15.7089L31.8476 2.9282C32.0529 2.72289 32.3233 2.61988 32.5929 2.61988C32.8626 2.61988 33.1329 2.72277 33.3382 2.9282L35.9755 5.56551C36.173 5.76321 36.2822 6.02793 36.2822 6.31082C36.2822 6.59372 36.173 6.85821 35.9755 7.0559L24.042 18.9898C24.0413 18.9904 24.0407 18.9912 24.04 18.9918L23.1953 19.8365C22.7845 20.2477 22.1153 20.2475 21.7045 19.8369ZM35.6181 9.7068C41.1367 17.4459 42.6142 18.9233 50.3535 24.4417L40.5809 34.2144C35.0618 26.4751 33.5843 24.9976 25.8458 19.4793L35.6181 9.7068ZM7.89914 56.9229C7.59246 57.2449 7.23984 57.4127 6.8223 57.4363C6.42586 57.4574 6.12129 57.3471 5.8616 57.0876L2.97257 54.1988C2.71288 53.9391 2.60249 53.634 2.62476 53.2381C2.64796 52.82 2.81531 52.4677 3.13734 52.1613L31.1076 25.5009C32.3895 26.6013 33.4587 27.6705 34.5592 28.9525L7.89914 56.9229ZM60.9672 58.021L59.543 53.1693C59.203 52.0109 58.1631 51.2323 56.9554 51.2323H36.5922C35.3844 51.2323 34.3445 52.0109 34.0046 53.1693L32.5804 58.021C32.5085 58.2659 32.5561 58.5307 32.7092 58.7353C32.8622 58.9399 33.1029 59.0602 33.3585 59.0602H60.1892C60.4447 59.0602 60.6854 58.9399 60.8385 58.7353C60.9915 58.5307 61.039 58.2659 60.9672 58.021ZM34.4415 57.4385L35.5607 53.6257C35.6979 53.1569 36.1028 52.8539 36.5922 52.8539H56.9554C57.4448 52.8539 57.8497 53.1569 57.9869 53.6259L59.106 57.4385H34.4415Z"
                    fill="currentColor" stroke="currentColor" stroke-width="0.3" />
                </svg>`,
          title: "Do injured workers have to be treated",
          subtitle: "Quis autem iure qui voluptate velit ess quam molestiae was."
        },
        {
          id: 2,
          delay: ".9s",
          bg: bg_law_4,
          icon: ` <svg width="66" height="47" viewBox="0 0 66 47" fill="none">
                  <path
                    d="M65.807 21.4782L58.576 2.00975C58.2901 1.23979 57.7196 0.626714 56.9697 0.28292C56.2195 -0.0602287 55.3831 -0.092455 54.6132 0.192944L49.7733 1.99029C48.18 2.58222 47.365 4.36023 47.9567 5.95351L48.0352 6.16492C46.8902 6.59959 45.6122 6.12483 43.6587 5.30666C40.742 4.08553 36.786 2.44584 31.7639 5.5869C27.6879 3.0499 24.4896 3.67574 21.6571 4.2397C20.2184 4.52562 18.848 4.79477 17.4327 4.64834C17.3929 4.27644 17.2909 3.90996 17.115 3.56617C16.7387 2.83166 16.1011 2.28819 15.3199 2.03618L10.4067 0.451272C9.62511 0.19926 8.79019 0.267838 8.05594 0.643213C7.32168 1.01949 6.77822 1.65706 6.52646 2.43824L0.149075 22.2032C-0.37261 23.8208 0.518907 25.5613 2.13668 26.083L7.04982 27.6684C7.35984 27.7686 7.67811 27.8182 7.99509 27.8182C8.47759 27.8182 8.95725 27.7033 9.4003 27.4765C9.67397 27.3363 9.91051 27.1495 10.1259 26.9427C10.1562 27.0922 10.1968 27.2401 10.2334 27.3886L9.0034 29.6881C8.47591 30.6749 8.37369 31.8714 8.72354 32.9705C9.06411 34.0407 9.78238 34.8953 10.7244 35.366C11.5454 35.8053 12.4498 35.9426 13.3086 35.816C13.0633 36.6295 13.0679 37.5217 13.3339 38.3577C13.6745 39.4279 14.3927 40.2828 15.3348 40.7535C16.3201 41.2804 17.4544 41.3904 18.5292 41.0653C18.9586 40.935 19.3575 40.7416 19.7151 40.4931C20.0601 41.5466 20.7728 42.3875 21.7042 42.8528C22.6893 43.3798 23.8235 43.4898 24.8983 43.1647C25.1369 43.0921 25.3664 43.0003 25.5841 42.8904C25.9073 44.1878 26.8299 45.3433 27.9798 45.9585C28.5291 46.252 29.176 46.4412 29.8474 46.4412C31.0677 46.4412 32.3688 45.8159 33.3101 44.0564L33.4937 43.713C33.5652 43.7504 33.6364 43.7879 33.7017 43.8221L34.1926 44.0781C34.7938 44.3876 35.3815 44.524 35.934 44.524C37.3124 44.524 38.4702 43.6745 39.0622 42.5463C39.4698 41.77 39.6133 40.8471 39.4155 39.9512C39.9128 39.9769 40.4157 39.9178 40.9072 39.7689C42.011 39.4344 42.9169 38.6933 43.4578 37.682C43.8948 36.865 44.0529 35.948 43.9212 35.0436C44.8444 35.4268 45.8655 35.4767 46.8392 35.1818C47.9526 34.8444 48.8663 34.0965 49.4118 33.0766C49.8577 32.2424 50.016 31.305 49.8743 30.3824C51.3791 30.9822 52.8309 30.7079 53.839 29.6035C54.861 28.4842 55.2046 26.7693 54.8224 25.2391C54.9014 25.2107 54.9815 25.182 55.0704 25.15L55.0846 25.145L55.1877 25.4224C55.6488 26.663 56.829 27.4314 58.0808 27.4314C58.4367 27.4314 58.7981 27.3695 59.1506 27.2387L63.9902 25.4413C65.5838 24.8494 66.3987 23.0718 65.807 21.4782ZM9.23259 25.1336C9.12663 25.4617 8.89757 25.7299 8.58716 25.8888C8.27624 26.0476 7.92497 26.0768 7.59742 25.9713L2.68402 24.3855C2.00236 24.1657 1.6266 23.4323 1.84639 22.7507L8.22403 2.98583C8.32973 2.65802 8.5588 2.38977 8.86908 2.23095C9.05664 2.13492 9.25954 2.08606 9.46308 2.08606C9.59611 2.08606 9.72901 2.10669 9.85856 2.14845C9.85882 2.14845 9.85882 2.14845 9.85882 2.14871L14.7722 3.73387C15.1 3.83958 15.3683 4.06864 15.5271 4.37892C15.6859 4.68933 15.7152 5.0406 15.6096 5.36867L10.5182 21.1491C10.4959 21.1922 10.4743 21.2363 10.459 21.284C10.4143 21.4219 10.3797 21.5615 10.3404 21.7003L9.23259 25.1336ZM11.5439 33.7818C11.0143 33.5173 10.6163 33.0371 10.4233 32.4297C10.2186 31.7866 10.2744 31.094 10.5763 30.5295L11.9659 27.9314C11.9662 27.9312 11.9666 27.9309 11.9667 27.9305L12.6304 26.6894C13.2488 25.5339 14.6816 25.0873 15.8443 25.6741C16.6071 26.2074 17.7511 27.3414 16.8831 28.9641L16.4101 29.8484L14.8273 32.8077C14.1992 33.9771 12.7372 34.4207 11.5439 33.7818ZM19.4395 38.1916C19.1376 38.7566 18.6304 39.1709 18.0114 39.3585C17.3928 39.546 16.7406 39.4827 16.1542 39.1695C15.6247 38.905 15.2266 38.4245 15.0336 37.8171C14.8289 37.1744 14.885 36.4816 15.1869 35.9173L16.4002 33.6493L17.983 30.69C18.2849 30.1253 18.7922 29.7111 19.4108 29.5235C20.0298 29.3355 20.6813 29.399 21.2465 29.7012C21.8113 30.0031 22.2255 30.5103 22.413 31.1293C22.6006 31.7479 22.5373 32.4001 22.235 32.9651L21.7905 33.7965C21.7903 33.7974 21.7896 33.7982 21.7891 33.7991L19.9833 37.1753C19.9833 37.1755 19.9833 37.1755 19.983 37.1758L19.4395 38.1916ZM24.3808 41.4577C23.7618 41.6453 23.1103 41.582 22.5236 41.2687C21.9941 41.0042 21.5963 40.524 21.403 39.9167C21.199 39.2745 21.2541 38.5833 21.5549 38.0192L21.5563 38.0167L23.3622 34.6405C23.3623 34.6401 23.3624 34.6397 23.3625 34.6393L23.3636 34.6376C23.6659 34.0741 24.1725 33.661 24.7902 33.4737C25.4085 33.2853 26.0607 33.349 26.6255 33.6514C27.7978 34.2785 28.2415 35.7426 27.6146 36.9149L26.7118 38.6032L25.9114 40.0995C25.9111 40.1 25.9108 40.1007 25.9105 40.1012L25.8089 40.2911C25.507 40.8556 24.9995 41.2701 24.3808 41.4577ZM28.8211 44.3855C27.6519 43.7602 26.8153 42.1916 27.4845 40.9405L28.2848 39.4442L28.2857 39.4421C28.5978 38.8604 29.1526 38.3929 29.8085 38.16C30.4632 37.9272 31.1205 37.9583 31.6599 38.2468C32.8351 38.8751 33.0247 39.4248 33.0647 39.7837C33.1257 40.3263 32.8959 41.0478 32.3209 42.1228C32.3205 42.1238 32.3202 42.1247 32.3199 42.1254L31.7371 43.2151C30.7499 45.0623 29.4868 44.7412 28.8211 44.3855ZM37.4828 41.7178C37.0622 42.5205 36.1343 43.0716 35.009 42.4927L34.5311 42.2432C34.4521 42.2015 34.3784 42.1628 34.3099 42.1271C34.6711 41.3238 34.9343 40.4508 34.8374 39.5853C34.7281 38.6094 34.1866 37.7955 33.204 37.109L36.7166 38.9953C37.8904 39.8195 37.8864 40.9489 37.4828 41.7178ZM52.5216 28.401C51.9473 29.0307 51.159 29.0841 50.178 28.5589L47.6342 27.1985C47.6324 27.1975 47.6307 27.1966 47.6289 27.1959C47.6029 27.182 47.5757 27.174 47.549 27.1629L39.3463 22.7103C38.9143 22.4748 38.3723 22.6357 38.1371 23.0688C37.9023 23.5017 38.0625 24.0428 38.4956 24.2779L46.8718 28.8247C47.4263 29.1498 47.838 29.6612 48.0275 30.2856C48.2267 30.9429 48.1596 31.6353 47.8385 32.2355C47.5177 32.8355 46.9788 33.2757 46.3215 33.4749C45.6631 33.6738 44.9713 33.607 44.3717 33.2862L41.8426 31.9334C41.7986 31.8976 41.7512 31.8649 41.6993 31.8371L35.5956 28.5736C35.1607 28.3408 34.6207 28.5057 34.3888 28.9397C34.1565 29.374 34.3202 29.9144 34.7549 30.1465L40.7052 33.3281C40.7493 33.3642 40.7971 33.397 40.8493 33.4249C41.4408 33.7411 41.8745 34.2718 42.0707 34.9198C42.2669 35.5676 42.2008 36.2497 41.8846 36.8409C41.5684 37.4323 41.0374 37.8657 40.3897 38.0623C39.7423 38.2582 39.0597 38.1924 38.4686 37.8762L37.1774 37.1857C37.1125 37.1511 37.0449 37.128 36.9767 37.1106L31.7656 34.3124C31.3315 34.0778 30.7906 34.2419 30.5581 34.6762C30.3249 35.11 30.4878 35.6508 30.9219 35.8837L32.0171 36.4717C31.2735 36.1977 30.4591 36.1458 29.6441 36.3487C29.8744 34.6602 29.0548 32.9276 27.4667 32.0781C26.4908 31.5561 25.3679 31.4426 24.3021 31.7576C24.2945 31.3744 24.2342 30.9895 24.1199 30.6118C23.7941 29.5367 23.0721 28.6547 22.0873 28.128C21.0973 27.599 19.9883 27.5078 18.9846 27.7877C18.9675 26.2725 18.0057 25.0125 16.8604 24.2083C16.8212 24.1782 16.7794 24.1511 16.7349 24.1274C15.025 23.2132 12.9584 23.6083 11.6917 24.9612C11.6712 24.0337 11.7981 23.1023 12.0558 22.1908L17.1462 6.41358C18.8669 6.61274 20.4591 6.29705 22.0045 5.98896C24.5739 5.47798 27.0141 4.99226 30.1673 6.71097C29.4803 7.25792 28.7894 7.88208 28.1024 8.57599C27.5587 9.1259 26.9995 9.67491 26.454 10.2105C24.4652 12.1638 22.7477 13.8508 22.2565 15.0481C22.1378 15.3375 22.1793 15.6679 22.366 15.9188C25.9158 20.6928 30.2178 19.0728 33.6731 17.7717C36.2636 16.7958 38.5009 15.9528 40.2832 17.3631C40.3168 17.3898 40.3523 17.4139 40.3894 17.4355C41.2406 17.9287 42.413 18.5076 43.7705 19.178C47.0526 20.7992 51.5477 23.0193 52.7092 24.7848C53.4898 25.9704 53.2421 27.6116 52.5216 28.401ZM54.0608 23.6178C52.5038 21.5062 48.1146 19.3341 44.5608 17.5792C43.2608 16.937 42.134 16.3808 41.3367 15.9229C38.7767 13.9443 35.8632 15.0412 33.0448 16.1026C29.7043 17.3599 26.7997 18.4533 24.1464 15.2967C24.7697 14.3653 26.2579 12.9038 27.704 11.4836C28.2556 10.9418 28.8206 10.3867 29.3705 9.83102C30.3234 8.8686 31.2769 8.05907 32.2066 7.41931C36.6995 4.32968 40.0301 5.72174 42.9696 6.95189C44.344 7.52733 45.7469 8.11463 47.1346 8.11463C47.6443 8.11463 48.1512 8.02156 48.6548 7.83348L54.4634 23.4729C54.3281 23.5217 54.191 23.571 54.0608 23.6178ZM63.3695 23.7692L58.5299 25.5665C57.8563 25.815 57.1087 25.4724 56.8596 24.8012L49.6289 5.33206C49.509 5.00927 49.5229 4.6571 49.6681 4.33999C49.8135 4.02301 50.0713 3.78234 50.3942 3.66233L55.2339 1.86499C55.3793 1.81098 55.5306 1.78404 55.6818 1.78404C55.8671 1.78404 56.052 1.82438 56.2265 1.90418C56.5432 2.04933 56.7838 2.3074 56.9037 2.63056L64.1347 22.0992C64.3842 22.7708 64.0408 23.5197 63.3695 23.7692Z"
                    fill="currentColor" />
                </svg>`,
          title: "Nobis occaecati nobis Repudiamus andaeum.",
          subtitle: "Quis autem iure qui voluptate velit ess quam molestiae was."
        },
        {
          id: 3,
          delay: "1.1s",
          bg: bg_law_5,
          icon: `<svg width="52" height="52" viewBox="0 0 52 52" fill="none">
                  <path
                    d="M26 0C24.5746 0 23.4034 1.17283 23.4034 2.59824V6.09205C22.4726 6.79758 21.8229 7.85743 21.6937 9.07625C19.6398 9.47747 18.0007 10.1084 16.3956 10.6844C14.7951 11.2587 13.1787 11.7613 11.0618 11.9928C10.6655 11.0617 9.73931 10.4034 8.67005 10.4034C7.65645 10.4034 6.77659 11.001 6.34766 11.8557C4.38011 11.4004 3.08073 10.5455 3.08073 10.5455C2.93675 10.4466 2.76623 10.3936 2.59154 10.3933C1.72695 10.3917 1.39331 11.5182 2.11927 11.9878C2.11927 11.9878 3.74243 13.0591 6.13776 13.5806C6.26704 14.134 6.57852 14.615 7.00104 14.972L0.0558612 33.4953C0.019096 33.5926 0.000176198 33.6958 2.59189e-06 33.7999C2.59189e-06 33.7999 -0.00929998 35.743 1.08672 37.6915C2.18276 39.64 4.50988 41.5999 8.66667 41.5999C12.8235 41.5999 15.1506 39.64 16.2466 37.6915C17.3426 35.743 17.3333 33.7999 17.3333 33.7999C17.3332 33.6958 17.3142 33.5926 17.2775 33.4953L10.3323 14.9737C10.7168 14.6487 11.0036 14.2194 11.1499 13.7261C13.5014 13.4785 15.3221 12.9116 16.9812 12.3162C18.5796 11.7426 20.0664 11.1758 21.8833 10.8062C22.4329 12.568 24.0629 13.8633 26 13.8633C27.9372 13.8633 29.5689 12.5681 30.1184 10.8062C31.9347 11.176 33.4204 11.7426 35.0187 12.3162C36.6781 12.9116 38.4997 13.4788 40.8518 13.7261C40.998 14.2192 41.2835 14.6488 41.6677 14.9737L34.7225 33.4953C34.6858 33.5926 34.6668 33.6958 34.6667 33.7999C34.6667 33.7999 34.6575 35.743 35.7534 37.6915C36.8494 39.64 39.1765 41.5999 43.3333 41.5999C47.4901 41.5999 49.8172 39.64 50.9133 37.6915C52.0093 35.743 52 33.7999 52 33.7999C51.9998 33.6958 51.981 33.5926 51.9441 33.4953L44.999 14.9737C45.4224 14.6165 45.7345 14.1348 45.8639 13.5806C48.2584 13.0589 49.8807 11.9878 49.8807 11.9878C50.6189 11.5113 50.2613 10.3653 49.3831 10.3933C49.2171 10.3998 49.0561 10.4515 48.9193 10.5455C48.9193 10.5455 47.6207 11.4003 45.654 11.8557C45.2252 11.0007 44.3455 10.4034 43.3316 10.4034C42.2624 10.4034 41.3362 11.0617 40.9398 11.9928C38.8223 11.7615 37.205 11.2587 35.6044 10.6844C33.9998 10.1086 32.3609 9.47764 30.308 9.07625C30.1789 7.85743 29.5291 6.79758 28.5983 6.09205V2.59824C28.5983 1.17283 27.4254 0 26 0H26ZM26 1.73496C26.4951 1.73496 26.8633 2.10313 26.8633 2.59824V5.28805C26.584 5.23105 26.2955 5.20148 26 5.20148C25.7021 5.20148 25.4113 5.23101 25.1299 5.28965V2.59824C25.1299 2.1031 25.5049 1.73496 26 1.73496ZM26 6.93504C27.4462 6.93504 28.5983 8.08707 28.5983 9.53328C28.5983 10.9795 27.4462 12.1299 26 12.1299C24.5538 12.1299 23.4034 10.9795 23.4034 9.53328C23.4034 8.08707 24.5538 6.93504 26 6.93504ZM8.67005 12.1299C9.15896 12.1299 9.53333 12.5111 9.53333 13C9.53333 13.4889 9.15896 13.8633 8.67005 13.8633C8.18114 13.8633 7.79831 13.4889 7.79831 13C7.79831 12.5111 8.18114 12.1299 8.67005 12.1299ZM43.3316 12.1299C43.8205 12.1299 44.2017 12.5111 44.2017 13C44.2017 13.4889 43.8205 13.8633 43.3316 13.8633C42.8427 13.8633 42.4684 13.4889 42.4684 13C42.4684 12.5111 42.8427 12.1299 43.3316 12.1299ZM24.2531 15.5847C23.7745 15.5946 23.394 15.9897 23.4034 16.4683V43.3316H17.3316C15.9062 43.3316 14.735 44.5096 14.735 45.935V46.7983C13.3096 46.7983 12.1299 47.9779 12.1299 49.4034V51.1299C12.1281 51.6112 12.5187 52.0018 13 52H39C39.4786 51.9981 39.8651 51.6085 39.8633 51.1299V49.4034C39.8633 47.9779 38.6921 46.7983 37.2667 46.7983V45.935C37.2667 44.5096 36.0955 43.3316 34.67 43.3316H28.5983V16.4683C28.6078 15.9791 28.2108 15.5789 27.7215 15.5847C27.2395 15.588 26.8539 15.9864 26.8633 16.4683V43.3316H25.1299V16.4683C25.1395 15.9791 24.7424 15.5789 24.2531 15.5847ZM8.62096 15.5945C8.63767 15.5949 8.65326 15.6011 8.67005 15.6011C8.68515 15.6011 8.69901 15.5978 8.71406 15.5945L15.2174 32.9363H2.11589L8.62096 15.5945ZM43.2876 15.5945C43.3027 15.5948 43.3164 15.6011 43.3316 15.6011C43.3477 15.6011 43.3627 15.5978 43.379 15.5945L49.8841 32.9363H36.7825L43.2876 15.5945ZM1.8349 34.6714H15.4984C15.4056 35.2269 15.2099 36.0019 14.7367 36.8431C13.8828 38.3613 12.3099 39.868 8.66667 39.868C5.02346 39.868 3.45058 38.3613 2.59662 36.8431C2.12347 36.0019 1.92776 35.2269 1.8349 34.6714ZM36.5016 34.6714H50.1651C50.0722 35.2269 49.8765 36.0019 49.4034 36.8431C48.5494 38.3613 46.9765 39.868 43.3333 39.868C39.6901 39.868 38.1172 38.3613 37.2633 36.8431C36.7901 36.0019 36.5944 35.2269 36.5016 34.6714ZM17.3316 45.0663C23.1696 45.1033 28.8122 45.0663 34.67 45.0663C35.1652 45.0663 35.5333 45.4412 35.5333 45.9363V46.7996H31.2017C30.045 46.7996 30.045 48.5347 31.2017 48.5347H37.2667C37.7618 48.5347 38.1299 48.9095 38.1299 49.4046V50.2679H13.8633V49.4046C13.8633 48.9095 14.2399 48.5347 14.735 48.5347H27.735C28.8917 48.5347 28.8917 46.7996 27.735 46.7996H16.4684V45.9363C16.4684 45.4412 16.8365 45.0663 17.3316 45.0663Z"
                    fill="currentColor" />
                </svg>`,
          title: "How Is equity determined in a divorce",
          subtitle: "Quis autem iure qui voluptate velit ess quam molestiae was."
        }
      ]
    };
  }
};
function _sfc_ssrRender$c(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_nuxt_link = __nuxt_component_0;
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "servivces__area pt-110 pb-90" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Bordered With Background Image</h3></div></div></div><div class="row"><!--[-->`);
  ssrRenderList($data.services_data, (item) => {
    _push(`<div class="col-xxl-4 col-lg-4 col-md-6"><div class="services__item-4 mb-30 transition-3 fix wow fadeInUp" data-sal="slide-up" data-sal-delay="130" data-sal-duration="1000"><div class="services__thumb-4 include-bg transition-3" style="${ssrRenderStyle({ backgroundImage: `url(${item.bg})` })}"></div><div class="services__icon-4 mb-30 transition-3"><span>${item.icon ?? ""}</span></div><div class="services__content-4 transition-3"><h4 class="services__title-4">`);
    _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(item.title)}`);
        } else {
          return [
            createTextVNode(toDisplayString(item.title), 1)
          ];
        }
      }),
      _: 2
    }, _parent));
    _push(`</h4><p>${ssrInterpolate(item.subtitle)}</p></div>`);
    _push(ssrRenderComponent(_component_nuxt_link, {
      href: "/service-details",
      class: "services-link-btn"
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`Learn More <i class="fa-regular fa-arrow-right"${_scopeId}></i>`);
        } else {
          return [
            createTextVNode("Learn More "),
            createVNode("i", { class: "fa-regular fa-arrow-right" })
          ];
        }
      }),
      _: 2
    }, _parent));
    _push(`</div></div>`);
  });
  _push(`<!--]--></div></div></section>`);
}
const _sfc_setup$c = _sfc_main$c.setup;
_sfc_main$c.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElBgImgStyle.vue");
  return _sfc_setup$c ? _sfc_setup$c(props, ctx) : void 0;
};
const ElBgImgStyle = /* @__PURE__ */ _export_sfc(_sfc_main$c, [["ssrRender", _sfc_ssrRender$c]]);
const _sfc_main$b = {
  components: { CreativeSingleFeature },
  data() {
    return {
      feature_data: [
        {
          id: 1,
          delay: ".3s",
          icon: `<svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                    <path d="M14.1046 25.2803H4.92805C2.76655 25.2803 0.998047 27.0488 0.998047 29.2103V41.0003H14.1046V25.2803Z" stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M23.2606 17.4199H18.014C15.8525 17.4199 14.084 19.1884 14.084 21.3499V41H27.1906V21.3499C27.1906 19.1884 25.4417 17.4199 23.2606 17.4199Z" stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M36.368 31.1748H27.1914V40.9998H40.298V35.1048C40.298 32.9433 38.5295 31.1748 36.368 31.1748Z" stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M21.671 1.83762L22.7124 3.92052C22.85 4.21527 23.2233 4.49036 23.5377 4.52966L25.4241 4.84407C26.6228 5.04057 26.9175 5.92478 26.0529 6.76974L24.5792 8.24349C24.3237 8.49894 24.1862 8.97056 24.2648 9.30461L24.6774 11.1124C25.0115 12.5469 24.2451 13.0971 22.9875 12.3504L21.219 11.3089C20.9046 11.1124 20.3741 11.1124 20.0597 11.3089L18.2912 12.3504C17.0336 13.0971 16.2672 12.5469 16.6013 11.1124L17.0139 9.30461C17.0925 8.97056 16.955 8.47929 16.6995 8.24349L15.2454 6.7894C14.3808 5.9248 14.6559 5.06018 15.8742 4.86368L17.7606 4.54933C18.075 4.49038 18.4484 4.21529 18.5859 3.94019L19.6274 1.85723C20.1972 0.717526 21.1011 0.717567 21.671 1.83762Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                 </svg>`,
          title: "Collect Website Leads With Embedded",
          subtitle: "Lorem Ipsum is that it more less normal distribution.",
          color: ""
        },
        {
          id: 2,
          delay: ".5s",
          icon: `<svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                    <path d="M17 5.99902V32.999C17 35.159 16.12 37.139 14.72 38.5789L14.64 38.659C14.46 38.839 14.26 39.019 14.08 39.159C13.48 39.679 12.8 40.0789 12.1 40.3589C11.88 40.4589 11.66 40.539 11.44 40.619C10.66 40.879 9.82 40.999 9 40.999C8.46 40.999 7.92002 40.939 7.40002 40.839C7.14002 40.779 6.87999 40.719 6.61999 40.639C6.29999 40.539 6.00001 40.4389 5.70001 40.2989C5.70001 40.2789 5.69999 40.2789 5.67999 40.2989C5.11999 40.0189 4.58002 39.699 4.08002 39.319L4.06 39.2989C3.8 39.0989 3.56003 38.899 3.34003 38.659C3.12003 38.419 2.89999 38.179 2.67999 37.919C2.29999 37.419 1.98001 36.879 1.70001 36.319C1.72001 36.299 1.72001 36.299 1.70001 36.299C1.70001 36.299 1.69999 36.279 1.67999 36.259C1.55999 35.979 1.45999 35.679 1.35999 35.379C1.27999 35.119 1.21997 34.859 1.15997 34.599C1.05997 34.079 1 33.539 1 32.999V5.99902C1 2.99902 3 0.999023 6 0.999023H12C15 0.999023 17 2.99902 17 5.99902Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M41 29.999V35.999C41 38.999 39 40.999 36 40.999H9C9.82 40.999 10.66 40.879 11.44 40.619C11.66 40.539 11.88 40.459 12.1 40.359C12.8 40.079 13.48 39.679 14.08 39.159C14.26 39.019 14.46 38.839 14.64 38.659L14.72 38.579L28.32 24.999H36C39 24.999 41 26.999 41 29.999Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M6.61938 40.6389C5.41938 40.2789 4.27941 39.6189 3.33942 38.6589C2.37942 37.7189 1.71937 36.5789 1.35938 35.3789C2.13937 37.8789 4.11938 39.8589 6.61938 40.6389Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M33.7388 19.5789L28.3188 24.9989L14.7188 38.5789C16.1187 37.1389 16.9988 35.1589 16.9988 32.9989V13.6789L22.4188 8.25894C24.5388 6.13895 27.3788 6.13895 29.4988 8.25894L33.7388 12.4989C35.8588 14.6189 35.8588 17.4589 33.7388 19.5789Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M9 34.999C10.1046 34.999 11 34.1036 11 32.999C11 31.8945 10.1046 30.999 9 30.999C7.89543 30.999 7 31.8945 7 32.999C7 34.1036 7.89543 34.999 9 34.999Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>`,
          title: "Product Design Event Development",
          subtitle: "Lorem Ipsum is that it more less normal distribution.",
          color: "has-green"
        },
        {
          id: 3,
          delay: ".7s",
          icon: `<svg width="43" height="42" viewBox="0 0 43 42" fill="none">
                    <path d="M1.99805 15V10C1.99805 5.02 6.01805 1 10.998 1H15.998" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M27.998 1H32.998C37.978 1 41.998 5.02 41.998 10V15" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M41.998 29V32C41.998 36.98 37.978 41 32.998 41H29.998" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M15.998 41H10.998C6.01805 41 1.99805 36.98 1.99805 32V27" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M31.998 16V26C31.998 30 29.998 32 25.998 32H17.998C13.998 32 11.998 30 11.998 26V16C11.998 12 13.998 10 17.998 10H25.998C29.998 10 31.998 12 31.998 16Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M35.9981 21H7.99805" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                 </svg>`,
          title: "Website Leads Hire in Fixed Contracting",
          subtitle: "Lorem Ipsum is that it more less normal distribution.",
          color: "has-pink"
        },
        {
          id: 4,
          delay: ".9s",
          icon: `<svg width="42" height="42" viewBox="0 0 42 42" fill="none">
                  <path d="M25.0001 29C25.0001 32.54 23.4601 35.74 21.0001 37.92C18.8801 39.84 16.0801 41 13.0001 41C6.38003 41 1 35.62 1 29C1 23.4799 4.76002 18.7999 9.84004 17.4199C11.22 20.8999 14.1801 23.58 17.8401 24.58C18.8401 24.86 19.9001 25 21.0001 25C22.1001 25 23.1601 24.86 24.1601 24.58C24.7001 25.94 25.0001 27.44 25.0001 29Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M33.0001 13.0001C33.0001 14.5601 32.7001 16.0601 32.1601 17.4201C30.7801 20.9001 27.8201 23.5801 24.1601 24.5801C23.1601 24.8601 22.1001 25.0001 21.0001 25.0001C19.9001 25.0001 18.84 24.8601 17.84 24.5801C14.18 23.5801 11.22 20.9001 9.84 17.4201C9.3 16.0601 9 14.5601 9 13.0001C9 6.38003 14.38 1 21.0001 1C27.6201 1 33.0001 6.38003 33.0001 13.0001Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M41.0001 29C41.0001 35.62 35.6201 41 29 41C25.92 41 23.12 39.84 21 37.92C23.46 35.74 25 32.54 25 29C25 27.44 24.7 25.94 24.16 24.58C27.82 23.58 30.78 20.8999 32.1601 17.4199C37.2401 18.7999 41.0001 23.4799 41.0001 29Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>`,
          title: "General are Consulting Process Embedded",
          subtitle: "Lorem Ipsum is that it more less normal distribution.",
          color: "has-purple"
        }
      ]
    };
  }
};
function _sfc_ssrRender$b(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_creative_single_feature = resolveComponent("creative-single-feature");
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "features__area p-relative z-index-1 grey-bg-7 pt-110 pb-90" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Colourful Icon Box</h3></div></div></div><div class="row"><!--[-->`);
  ssrRenderList($data.feature_data, (item) => {
    _push(`<div class="col-xxl-3 col-xl-3 col-lg-4 col-md-6 col-sm-6">`);
    _push(ssrRenderComponent(_component_creative_single_feature, { item }, null, _parent));
    _push(`</div>`);
  });
  _push(`<!--]--></div></div></section>`);
}
const _sfc_setup$b = _sfc_main$b.setup;
_sfc_main$b.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElClrIconBox.vue");
  return _sfc_setup$b ? _sfc_setup$b(props, ctx) : void 0;
};
const ElClrIconBox = /* @__PURE__ */ _export_sfc(_sfc_main$b, [["ssrRender", _sfc_ssrRender$b]]);
const _sfc_main$a = {};
function _sfc_ssrRender$a(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "support__area p-relative pt-110 pb-60 z-index-1" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Icon Box Style 5</h3></div></div></div><div class="row"><div class="col-xl-3 col-lg-4 col-md-6 col-sm-6"><div class="support__item mb-55"><div class="support__icon"><span><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M18.4698 16.83L18.8598 19.99C18.9598 20.82 18.0698 21.4 17.3598 20.97L13.1698 18.48C12.7098 18.48 12.2599 18.45 11.8199 18.39C12.5599 17.52 12.9998 16.42 12.9998 15.23C12.9998 12.39 10.5398 10.09 7.49985 10.09C6.33985 10.09 5.26985 10.42 4.37985 11C4.34985 10.75 4.33984 10.5 4.33984 10.24C4.33984 5.68999 8.28985 2 13.1698 2C18.0498 2 21.9998 5.68999 21.9998 10.24C21.9998 12.94 20.6098 15.33 18.4698 16.83Z" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M13 15.2298C13 16.4198 12.56 17.5198 11.82 18.3898C10.83 19.5898 9.26 20.3598 7.5 20.3598L4.89 21.9098C4.45 22.1798 3.89 21.8098 3.95 21.2998L4.2 19.3298C2.86 18.3998 2 16.9098 2 15.2298C2 13.4698 2.94 11.9198 4.38 10.9998C5.27 10.4198 6.34 10.0898 7.5 10.0898C10.54 10.0898 13 12.3898 13 15.2298Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="support__content"><h3 class="support__title">No meetings</h3><p>Quis nostrud exercitation ullamco laboris nisi ut aliquip ex consequat.</p></div></div></div><div class="col-xl-3 col-lg-4 col-md-6 col-sm-6"><div class="support__item mb-55"><div class="support__icon"><span><svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15.63 6.39014C15.63 6.39014 15.66 5.44014 15.63 5.13014C15.46 3.09014 14.13 2.39014 11.52 2.39014H5.21C2.05 2.39014 1 3.44014 1 6.60014V15.0201C1 16.2801 1.38 17.5501 2.37 18.3601L3 18.8101" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M15.7398 9.75977V15.0198C15.7398 18.1798 14.6898 19.2298 11.5298 19.2298H6.25977" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M21.0002 5.5498V14.6198C21.0002 16.2898 19.8802 16.8698 18.5202 15.9098L15.7402 13.9598" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M21.0195 1L1.01953 21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="support__content"><h3 class="support__title">24/7 support </h3><p>Quis nostrud exercitation ullamco laboris nisi ut aliquip ex consequat.</p></div></div></div><div class="col-xl-3 col-lg-4 col-md-6 col-sm-6"><div class="support__item mb-55"><div class="support__icon"><span><svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15.63 6.39014C15.63 6.39014 15.66 5.44014 15.63 5.13014C15.46 3.09014 14.13 2.39014 11.52 2.39014H5.21C2.05 2.39014 1 3.44014 1 6.60014V15.0201C1 16.2801 1.38 17.5501 2.37 18.3601L3 18.8101" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M15.7398 9.75977V15.0198C15.7398 18.1798 14.6898 19.2298 11.5298 19.2298H6.25977" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M21.0002 5.5498V14.6198C21.0002 16.2898 19.8802 16.8698 18.5202 15.9098L15.7402 13.9598" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M21.0195 1L1.01953 21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="support__content"><h3 class="support__title">24/7 support </h3><p>Quis nostrud exercitation ullamco laboris nisi ut aliquip ex consequat.</p></div></div></div><div class="col-xl-3 col-lg-4 col-md-6 col-sm-6"><div class="support__item mb-55"><div class="support__icon"><span><svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15.63 6.39014C15.63 6.39014 15.66 5.44014 15.63 5.13014C15.46 3.09014 14.13 2.39014 11.52 2.39014H5.21C2.05 2.39014 1 3.44014 1 6.60014V15.0201C1 16.2801 1.38 17.5501 2.37 18.3601L3 18.8101" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M15.7398 9.75977V15.0198C15.7398 18.1798 14.6898 19.2298 11.5298 19.2298H6.25977" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M21.0002 5.5498V14.6198C21.0002 16.2898 19.8802 16.8698 18.5202 15.9098L15.7402 13.9598" stroke="#04121F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M21.0195 1L1.01953 21" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="support__content"><h3 class="support__title">24/7 support </h3><p>Quis nostrud exercitation ullamco laboris nisi ut aliquip ex consequat.</p></div></div></div></div></div></section>`);
}
const _sfc_setup$a = _sfc_main$a.setup;
_sfc_main$a.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElIconBoxStyleFive.vue");
  return _sfc_setup$a ? _sfc_setup$a(props, ctx) : void 0;
};
const ElIconBoxStyleFive = /* @__PURE__ */ _export_sfc(_sfc_main$a, [["ssrRender", _sfc_ssrRender$a]]);
const _sfc_main$9 = {
  mixins: [SalScrollAnimationMixin],
  data() {
    return {
      service_data: [
        {
          id: 1,
          delay: "130",
          icon: `<svg width="28" height="28" viewBox="0 0 28 28" fill="none">
                  <path d="M20.1296 23.0769H7.96432C7.42077 23.0769 6.8125 22.6483 6.63132 22.1287L1.2734 7.08594C0.509833 4.92955 1.40282 4.26705 3.24056 5.59206L8.28787 9.21634C9.12909 9.8009 10.0868 9.50212 10.4492 8.55384L12.7269 2.4614C13.4517 0.512865 14.6552 0.512865 15.38 2.4614L17.6578 8.55384C18.0201 9.50212 18.9778 9.8009 19.8061 9.21634L24.5428 5.82588C26.5617 4.37097 27.5324 5.11142 26.7041 7.46265L21.4756 22.1546C21.2815 22.6483 20.6732 23.0769 20.1296 23.0769Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path opacity="0.5" d="M6.92871 26.9995H21.1647" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path opacity="0.5" d="M10.8105 16.6074H17.2815" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>`,
          title: "Beautiful Pages",
          subtitle: "Understand the business model of your product. Know your customers, value propositions."
        },
        {
          id: 2,
          delay: "150",
          icon: `<svg width="28" height="28" viewBox="0 0 28 28" fill="none">
                  <path opacity="0.4" d="M23.1002 8.8C25.2541 8.8 27.0002 7.05391 27.0002 4.9C27.0002 2.74609 25.2541 1 23.1002 1C20.9463 1 19.2002 2.74609 19.2002 4.9C19.2002 7.05391 20.9463 8.8 23.1002 8.8Z" stroke="#04121F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path opacity="0.4" d="M7.5 15.3003H14" stroke="#04121F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path opacity="0.4" d="M7.5 20.5H19.2" stroke="#04121F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M16.6 1H10.1C3.6 1 1 3.6 1 10.1V17.9C1 24.4 3.6 27 10.1 27H17.9C24.4 27 27 24.4 27 17.9V11.4" stroke="#04121F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>`,
          title: "Easy Invoicing",
          subtitle: "Understand the business model of your product. Know your customers, value propositions."
        },
        {
          id: 3,
          delay: "170",
          icon: `<svg width="28" height="28" viewBox="0 0 28 28" fill="none">
                  <path d="M11.4 4.25V21.8C11.4 23.204 10.828 24.491 9.91798 25.427L9.86601 25.479C9.74901 25.596 9.61901 25.713 9.50201 25.804C9.11201 26.142 8.66998 26.402 8.21498 26.584C8.07198 26.649 7.929 26.701 7.786 26.753C7.279 26.922 6.733 27 6.2 27C5.849 27 5.49802 26.961 5.16002 26.896C4.99102 26.857 4.822 26.818 4.653 26.766C4.445 26.701 4.25001 26.636 4.05501 26.545C4.05501 26.532 4.055 26.532 4.042 26.545C3.678 26.363 3.32701 26.155 3.00201 25.908L2.989 25.895C2.82 25.765 2.66402 25.635 2.52102 25.479C2.37802 25.323 2.235 25.167 2.092 24.998C1.845 24.673 1.63701 24.322 1.45501 23.958C1.46801 23.945 1.46801 23.945 1.45501 23.945C1.45501 23.945 1.455 23.932 1.442 23.919C1.364 23.737 1.29899 23.542 1.23399 23.347C1.18199 23.178 1.14298 23.009 1.10398 22.84C1.03898 22.502 1 22.151 1 21.8V4.25C1 2.3 2.3 1 4.25 1H8.15C10.1 1 11.4 2.3 11.4 4.25Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path opacity="0.5" d="M27.0002 19.8501V23.7501C27.0002 25.7001 25.7002 27.0001 23.7502 27.0001H6.2002C6.7332 27.0001 7.2792 26.9221 7.7862 26.7531C7.9292 26.7011 8.07218 26.6491 8.21518 26.5841C8.67018 26.4021 9.11221 26.1421 9.50221 25.8041C9.61921 25.7131 9.74921 25.5961 9.86621 25.4791L9.91818 25.4271L18.7582 16.6001H23.7502C25.7002 16.6001 27.0002 17.9001 27.0002 19.8501Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M4.65338 26.7662C3.87338 26.5322 3.1324 26.1032 2.5214 25.4792C1.8974 24.8682 1.46838 24.1272 1.23438 23.3472C1.74137 24.9722 3.02838 26.2592 4.65338 26.7662Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M22.281 13.077L18.758 16.6L9.91797 25.427C10.828 24.491 11.4 23.204 11.4 21.8V9.24204L14.923 5.71905C16.301 4.34105 18.147 4.34105 19.525 5.71905L22.281 8.47504C23.659 9.85304 23.659 11.699 22.281 13.077Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  <path opacity="0.4" d="M6.19941 23.1C6.91738 23.1 7.49941 22.518 7.49941 21.8C7.49941 21.082 6.91738 20.5 6.19941 20.5C5.48144 20.5 4.89941 21.082 4.89941 21.8C4.89941 22.518 5.48144 23.1 6.19941 23.1Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>`,
          title: "CMS & Style Guide",
          subtitle: "Understand the business model of your product. Know your customers, value propositions."
        }
      ]
    };
  }
};
function _sfc_ssrRender$9(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_nuxt_link = __nuxt_component_0;
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "services__area p-relative z-index-1 pt-110 pb-60" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Icon Box Style 4</h3></div></div></div><div class="services__item-wrapper-7"><div class="row"><!--[-->`);
  ssrRenderList($data.service_data, (item) => {
    _push(`<div class="col-xxl-4 col-xl-4 col-lg-4 col-md-6"><div class="services__item-7 text-center mb-60" data-sal="slide-up"${ssrRenderAttr("data-sal-delay", item.delay)} data-sal-duration="1000"><div class="services__icon-7"><span>${item.icon ?? ""}</span></div><div class="services__content-7"><h3 class="services__title-7">`);
    _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(item.title)}`);
        } else {
          return [
            createTextVNode(toDisplayString(item.title), 1)
          ];
        }
      }),
      _: 2
    }, _parent));
    _push(`</h3><p>${ssrInterpolate(item.subtitle)}</p><div class="services__btn-7">`);
    _push(ssrRenderComponent(_component_nuxt_link, {
      href: "/service-details",
      class: "tp-link-btn-3"
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(` Learn More <span${_scopeId}><i class="fa-regular fa-arrow-right"${_scopeId}></i></span>`);
        } else {
          return [
            createTextVNode(" Learn More "),
            createVNode("span", null, [
              createVNode("i", { class: "fa-regular fa-arrow-right" })
            ])
          ];
        }
      }),
      _: 2
    }, _parent));
    _push(`</div></div></div></div>`);
  });
  _push(`<!--]--></div></div></div></section>`);
}
const _sfc_setup$9 = _sfc_main$9.setup;
_sfc_main$9.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElIconBoxStyleFour.vue");
  return _sfc_setup$9 ? _sfc_setup$9(props, ctx) : void 0;
};
const ElIconBoxStyleFour = /* @__PURE__ */ _export_sfc(_sfc_main$9, [["ssrRender", _sfc_ssrRender$9]]);
const _sfc_main$8 = {};
function _sfc_ssrRender$8(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "product-wrapper pt-110 pb-80 grey-bg-4" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Icon Box Style 7</h3></div></div></div><div class="row"><div class="col-lg-3 col-md-6 col-sm-6"><div class="product__features-item mb-35"><div class="product__features-icon"><span><svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.15399 20.8347V17.3131C5.15399 16.1432 6.06589 15.094 7.36176 15.094C8.52565 15.094 9.56954 16.0106 9.56954 17.3131V20.702C9.56954 23.0538 7.62574 25.0076 5.28597 25.0076C2.94621 25.0076 1.0024 23.0417 1.0024 20.702V13.2729C0.870414 6.49501 6.19788 1.00757 12.9412 1.00757C19.6845 1.00757 25 6.495 25 13.1402V20.5694C25 22.9211 23.0562 24.8749 20.7164 24.8749C18.3767 24.8749 16.4329 22.9211 16.4329 20.5694V17.1804C16.4329 16.0106 17.3448 14.9613 18.6406 14.9613C19.8045 14.9613 20.8484 15.8779 20.8484 17.1804V20.8347" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="product__features-content"><h3 class="product__features-title">Ergonomic design</h3><p>Ultra-comfortable ultra-light</p></div></div></div><div class="col-lg-3 col-md-6 col-sm-6"><div class="product__features-item mb-35"><div class="product__features-icon"><span><svg width="28" height="21" viewBox="0 0 28 21" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14.0024 13.235C15.4384 13.235 16.6025 11.8655 16.6025 10.1761C16.6025 8.48672 15.4384 7.11719 14.0024 7.11719C12.5664 7.11719 11.4023 8.48672 11.4023 10.1761C11.4023 11.8655 12.5664 13.235 14.0024 13.235Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M24.4004 19.3536C26.0384 16.7994 27.0005 13.6181 27.0005 10.1768C27.0005 6.73549 26.0384 3.55421 24.4004 1" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M3.60006 1C1.96202 3.55421 1 6.73549 1 10.1768C1 13.6181 1.96202 16.7994 3.60006 19.3536" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M20.2402 15.6826C21.2153 14.1531 21.8003 12.2413 21.8003 10.1765C21.8003 8.11171 21.2153 6.19987 20.2402 4.67041" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M7.75732 4.67041C6.7823 6.19987 6.19727 8.11171 6.19727 10.1765C6.19727 12.2413 6.7823 14.1531 7.75732 15.6826" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="product__features-content"><h3 class="product__features-title">Response frequency</h3><p>4 Hz – 40,000 Hz</p></div></div></div><div class="col-lg-3 col-md-6 col-sm-6"><div class="product__features-item mb-35"><div class="product__features-icon"><span><svg width="18" height="26" viewBox="0 0 18 26" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 20.9684L16.5297 7.38372C17.171 6.81969 17.1585 5.91963 16.4795 5.3796L11.6382 1.52741C10.3807 0.531363 9.34958 0.987394 9.34958 2.54747V23.4525C9.34958 25.0126 10.3807 25.4686 11.6382 24.4726L16.4795 20.6204C17.1585 20.0804 17.171 19.1803 16.5297 18.6163L1 5.03157" stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="product__features-content"><h3 class="product__features-title">Bluetooth</h3><p>Bluetooth version 5.2</p></div></div></div><div class="col-lg-3 col-md-6 col-sm-6"><div class="product__features-item mb-35"><div class="product__features-icon"><span><svg width="27" height="22" viewBox="0 0 27 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 8.45042V13.5521C1 16.1029 2.25 17.3783 4.75 17.3783H6.5375C7 17.3783 7.4625 17.5186 7.8625 17.7609L11.5125 20.0949C14.6625 22.1101 17.25 20.6433 17.25 16.8554V5.14711C17.25 1.34638 14.6625 -0.107584 11.5125 1.90756L7.8625 4.24156C7.4625 4.48389 7 4.62419 6.5375 4.62419H4.75C2.25 4.62419 1 5.8996 1 8.45042Z" stroke="currentColor" stroke-width="1.5"></path><path d="M21.002 5.89844C23.227 8.92116 23.227 13.079 21.002 16.1017" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M23.2871 2.71069C26.8996 7.62102 26.8996 14.3807 23.2871 19.291" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="product__features-content"><h3 class="product__features-title">Sound optimization</h3><p>HD Noise Canceling Processor QN1,</p></div></div></div></div></div></div>`);
}
const _sfc_setup$8 = _sfc_main$8.setup;
_sfc_main$8.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElIconBoxStyleSeven.vue");
  return _sfc_setup$8 ? _sfc_setup$8(props, ctx) : void 0;
};
const ElIconBoxStyleSeven = /* @__PURE__ */ _export_sfc(_sfc_main$8, [["ssrRender", _sfc_ssrRender$8]]);
const _sfc_main$7 = {
  mixins: [SalScrollAnimationMixin],
  data() {
    return {
      service_items: [
        {
          delay: ".3s",
          icon: polity_ser_icon_1,
          sm_title: "Main Mission",
          title: "Core Principles",
          desc: "We are committed to restoring honesty, integrity, government. Explore our principles."
        },
        {
          delay: ".5s",
          icon: polity_ser_icon_2,
          sm_title: "What We Do",
          title: "Political Program",
          desc: "We are committed to restoring honesty, integrity, government. Explore our principles."
        },
        {
          delay: ".7s",
          icon: polity_ser_icon_3,
          sm_title: "Our Mission",
          title: "Our Members",
          desc: "We are committed to restoring honesty, integrity, government. Explore our principles."
        }
      ]
    };
  }
};
function _sfc_ssrRender$7(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_nuxt_link = __nuxt_component_0;
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "features__area pt-110 pb-85" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Icon Box Style 6</h3></div></div></div><div class="row"><!--[-->`);
  ssrRenderList($data.service_items, (item, i) => {
    _push(`<div class="col-xxl-4 col-xl-4 col-lg-4 col-md-6"><div class="features__item-10 transition-3 mb-30" data-sal="slide-up" data-sal-delay="130" data-sal-duration="1000"><div class="features__icon-10"><span><img${ssrRenderAttr("src", item.icon)} alt=""></span></div><div class="features__content-10"><span>${ssrInterpolate(item.sm_title)}</span><h3 class="features__title-10">${ssrInterpolate(item.title)}</h3><p>${ssrInterpolate(item.desc)}</p><div class="features__btn-10">`);
    _push(ssrRenderComponent(_component_nuxt_link, {
      href: "/service-details",
      class: "tp-btn-border-9"
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(` Learn More <i class="fa-regular fa-angle-right"${_scopeId}></i>`);
        } else {
          return [
            createTextVNode(" Learn More "),
            createVNode("i", { class: "fa-regular fa-angle-right" })
          ];
        }
      }),
      _: 2
    }, _parent));
    _push(`</div></div></div></div>`);
  });
  _push(`<!--]--></div></div></section>`);
}
const _sfc_setup$7 = _sfc_main$7.setup;
_sfc_main$7.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElIconBoxStyleSix.vue");
  return _sfc_setup$7 ? _sfc_setup$7(props, ctx) : void 0;
};
const ElIconBoxStyleSix = /* @__PURE__ */ _export_sfc(_sfc_main$7, [["ssrRender", _sfc_ssrRender$7]]);
const _sfc_main$6 = {
  mixins: [SalScrollAnimationMixin],
  data() {
    return {
      feature_data: [
        {
          id: 1,
          delay: "130",
          icon: icon_1,
          title: "Manage Expenses",
          subtitle: "Build an online store that rank higher & sell more and integrates easily."
        },
        {
          id: 2,
          delay: "150",
          icon: icon_2,
          title: "Responsive any device",
          subtitle: "Build an online store that rank higher & sell more and integrates easily."
        },
        {
          id: 3,
          delay: "170",
          icon: icon_3,
          title: "World-class support",
          subtitle: "Build an online store that rank higher & sell more and integrates easily."
        }
      ]
    };
  }
};
function _sfc_ssrRender$6(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_nuxt_link = __nuxt_component_0;
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "features__area p-relative z-index-1 pb-90 pt-110" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Icon Box Style 3</h3></div></div></div><div class="row"><!--[-->`);
  ssrRenderList($data.feature_data, (item) => {
    _push(`<div class="col-xxl-4 col-xl-4 col-lg-4 col-md-6"><div class="features__item-7 text-center mb-30 white-bg transition-3" data-sal="slide-up"${ssrRenderAttr("data-sal-delay", item.delay)} data-sal-duration="1000"><div class="features__icon-7"><span><img${ssrRenderAttr("src", item.icon)} alt="icon"></span></div><div class="features__content-7"><h3 class="features__title-7">${ssrInterpolate(item.title)}</h3><p>${ssrInterpolate(item.subtitle)}</p><div class="features__btn-7">`);
    _push(ssrRenderComponent(_component_nuxt_link, {
      href: "/service-details",
      class: "tp-link-btn-3"
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(` Read More <span${_scopeId}><i class="fa-regular fa-arrow-right"${_scopeId}></i></span>`);
        } else {
          return [
            createTextVNode(" Read More "),
            createVNode("span", null, [
              createVNode("i", { class: "fa-regular fa-arrow-right" })
            ])
          ];
        }
      }),
      _: 2
    }, _parent));
    _push(`</div></div></div></div>`);
  });
  _push(`<!--]--></div></div></section>`);
}
const _sfc_setup$6 = _sfc_main$6.setup;
_sfc_main$6.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElIconBoxStyleThree.vue");
  return _sfc_setup$6 ? _sfc_setup$6(props, ctx) : void 0;
};
const ElIconBoxStyleThree = /* @__PURE__ */ _export_sfc(_sfc_main$6, [["ssrRender", _sfc_ssrRender$6]]);
const _sfc_main$5 = {
  mixins: [SalScrollAnimationMixin]
};
function _sfc_ssrRender$5(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<div${ssrRenderAttrs(mergeProps({ class: "col-xxl-3 col-xl-3 col-lg-4 col-md-6" }, _attrs))}><div class="about__features"><div class="about__features-item d-flex align-items-start" data-sal="slide-up" data-sal-delay="130" data-sal-duration="1000"><div class="about__features-icon"><span><svg width="25" height="25" viewBox="0 0 25 25" fill="none"><path d="M12.0037 23.7486C11.4037 23.7486 10.8137 23.6086 10.3337 23.3386L5.60367 20.6086C3.64367 19.2886 3.51367 19.0486 3.51367 16.9786V12.0186C3.51367 9.94859 3.64367 9.70859 5.56367 8.41859L10.3437 5.65859C11.2937 5.10859 12.7237 5.10859 13.6737 5.65859L18.4037 8.38859C20.3637 9.70859 20.4937 9.94859 20.4937 12.0186V16.9786C20.4937 19.0486 20.3637 19.2886 18.4437 20.5786L13.6637 23.3386C13.1937 23.6186 12.5937 23.7486 12.0037 23.7486ZM12.0037 6.74859C11.6637 6.74859 11.3237 6.81859 11.0837 6.95859L6.35367 9.68859C5.01367 10.5986 5.01367 10.5986 5.01367 12.0186V16.9786C5.01367 18.3986 5.01367 18.3986 6.40367 19.3386L11.0937 22.0386C11.5737 22.3186 12.4437 22.3186 12.9237 22.0386L17.6537 19.3086C18.9937 18.3986 18.9937 18.3986 18.9937 16.9786V12.0186C18.9937 10.5986 18.9937 10.5986 17.6037 9.65859L12.9137 6.95859C12.6837 6.81859 12.3437 6.74859 12.0037 6.74859Z" fill="currentColor"></path><path d="M17.502 9.37854C17.092 9.37854 16.752 9.03854 16.752 8.62854V5.99854C16.752 4.41854 16.082 3.74854 14.502 3.74854H9.50195C7.92195 3.74854 7.25195 4.41854 7.25195 5.99854V8.55854C7.25195 8.96853 6.91195 9.30854 6.50195 9.30854C6.09195 9.30854 5.75195 8.97854 5.75195 8.55854V5.99854C5.75195 3.57854 7.08195 2.24854 9.50195 2.24854H14.502C16.922 2.24854 18.252 3.57854 18.252 5.99854V8.62854C18.252 9.03854 17.912 9.37854 17.502 9.37854Z" fill="currentColor"></path><path d="M13.6302 18.6287C13.4202 18.6287 13.2002 18.5887 12.9802 18.4987L12.0002 18.1187L11.0202 18.5087C10.4902 18.7187 9.95024 18.6687 9.55024 18.3787C9.15024 18.0887 8.94024 17.5887 8.97024 17.0187L9.03024 15.9687L8.36024 15.1587C8.00024 14.7087 7.88024 14.1887 8.04024 13.7087C8.19024 13.2387 8.60024 12.8787 9.15024 12.7387L10.1702 12.4787L10.7402 11.5887C11.3502 10.6287 12.6602 10.6287 13.2702 11.5887L13.8402 12.4787L14.8602 12.7387C15.4102 12.8787 15.8202 13.2387 15.9702 13.7087C16.1202 14.1787 16.0002 14.7087 15.6402 15.1487L14.9702 15.9587L15.0302 17.0087C15.0602 17.5787 14.8502 18.0687 14.4502 18.3687C14.2102 18.5387 13.9302 18.6287 13.6302 18.6287ZM9.52024 14.1987L10.1902 15.0087C10.4202 15.2787 10.5502 15.7087 10.5302 16.0587L10.4702 17.1087L11.4502 16.7187C11.7802 16.5887 12.2202 16.5887 12.5502 16.7187L13.5302 17.1087L13.4702 16.0587C13.4502 15.7087 13.5802 15.2887 13.8102 15.0087L14.4802 14.1987L13.4602 13.9387C13.1202 13.8487 12.7602 13.5887 12.5702 13.2987L12.0102 12.4187L11.4402 13.2987C11.2502 13.5987 10.8902 13.8587 10.5502 13.9487L9.52024 14.1987Z" fill="currentColor"></path></svg></span></div><div class="about__features-content"><h3 class="about__features-title">Leadership</h3><p>Fully committed to the success company</p></div></div><div class="about__features-item d-flex align-items-start" data-sal="slide-up" data-sal-delay="150" data-sal-duration="1000"><div class="about__features-icon yellow-color"><span><svg class="center-icon" width="25" height="25" viewBox="0 0 25 25" fill="none"><path d="M5.00195 15.9985V8.99854" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M5.25195 22.9985C7.04688 22.9985 8.50195 21.5435 8.50195 19.7485C8.50195 17.9536 7.04688 16.4985 5.25195 16.4985C3.45703 16.4985 2.00195 17.9536 2.00195 19.7485C2.00195 21.5435 3.45703 22.9985 5.25195 22.9985Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M5.00195 8.99854C6.65881 8.99854 8.00195 7.65539 8.00195 5.99854C8.00195 4.34168 6.65881 2.99854 5.00195 2.99854C3.3451 2.99854 2.00195 4.34168 2.00195 5.99854C2.00195 7.65539 3.3451 8.99854 5.00195 8.99854Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M19.002 8.99854C20.6588 8.99854 22.002 7.65539 22.002 5.99854C22.002 4.34168 20.6588 2.99854 19.002 2.99854C17.3451 2.99854 16.002 4.34168 16.002 5.99854C16.002 7.65539 17.3451 8.99854 19.002 8.99854Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M5.13086 15.9985C5.58086 14.2485 7.18086 12.9485 9.07086 12.9585L12.5009 12.9685C15.1209 12.9785 17.3509 11.2985 18.1709 8.9585" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="about__features-content"><h3 class="about__features-title">Responsibility</h3><p>Employees will always be my top prioty</p></div></div><div class="about__features-item d-flex align-items-start" data-sal="slide-up" data-sal-delay="170" data-sal-duration="1000"><div class="about__features-icon purple-color"><span><svg width="25" height="25" viewBox="0 0 25 25" fill="none"><path d="M9.10159 14.5344C9.10159 14.1202 8.76581 13.7844 8.35159 13.7844H4.64392C3.89249 13.7844 3.7619 13.5515 3.76116 13.5498C3.75967 13.5466 3.67207 13.2926 4.16481 12.7241L13.246 2.40728C13.2461 2.4072 13.2461 2.40712 13.2462 2.40705C13.5405 2.07296 13.7834 1.89024 13.9556 1.80534C14.0536 1.757 14.1081 1.74921 14.1301 1.74876C14.1462 1.76358 14.1814 1.80495 14.2224 1.90432C14.2954 2.08082 14.357 2.37782 14.357 2.82342V11.4627C14.357 11.8769 14.6928 12.2127 15.107 12.2127H18.8147C19.5715 12.2127 19.7048 12.4482 19.706 12.4508C19.7069 12.4527 19.7955 12.7065 19.2971 13.2692L19.2956 13.2709L10.2126 23.5898C9.91817 23.924 9.67521 24.1068 9.50303 24.1917C9.40502 24.2401 9.35053 24.2479 9.32853 24.2483C9.31242 24.2335 9.27723 24.1921 9.23617 24.0927C9.16324 23.9162 9.10159 23.6192 9.10159 23.1736V14.5344ZM14.1417 1.74959C14.1417 1.74969 14.1404 1.74959 14.1378 1.74894C14.1405 1.74916 14.1418 1.74948 14.1417 1.74959ZM14.1242 1.7438C14.1218 1.74257 14.1206 1.74173 14.1207 1.74162C14.1207 1.74151 14.1219 1.74212 14.1242 1.7438ZM9.31688 24.2475C9.31692 24.2474 9.31826 24.2475 9.32079 24.2481C9.3181 24.2479 9.31683 24.2476 9.31688 24.2475ZM9.33442 24.2533C9.33685 24.2545 9.33796 24.2553 9.33791 24.2554C9.33785 24.2556 9.33664 24.2549 9.33442 24.2533Z" stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="about__features-content"><h3 class="about__features-title">Flexibility</h3><p>The ability to switch is an important skill</p></div></div></div></div>`);
}
const _sfc_setup$5 = _sfc_main$5.setup;
_sfc_main$5.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElListIconBoxItem.vue");
  return _sfc_setup$5 ? _sfc_setup$5(props, ctx) : void 0;
};
const ElListIconBoxItem = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["ssrRender", _sfc_ssrRender$5]]);
const _sfc_main$4 = {
  components: { ElListIconBoxItem }
};
function _sfc_ssrRender$4(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_el_list_icon_box_item = resolveComponent("el-list-icon-box-item");
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "about__area pt-110 pb-120 p-relative" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">List Icon Box</h3></div></div></div><div class="row">`);
  _push(ssrRenderComponent(_component_el_list_icon_box_item, null, null, _parent));
  _push(ssrRenderComponent(_component_el_list_icon_box_item, null, null, _parent));
  _push(ssrRenderComponent(_component_el_list_icon_box_item, null, null, _parent));
  _push(ssrRenderComponent(_component_el_list_icon_box_item, null, null, _parent));
  _push(`</div></div></section>`);
}
const _sfc_setup$4 = _sfc_main$4.setup;
_sfc_main$4.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElListIconBox.vue");
  return _sfc_setup$4 ? _sfc_setup$4(props, ctx) : void 0;
};
const ElListIconBox = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["ssrRender", _sfc_ssrRender$4]]);
const _sfc_main$3 = {
  mixins: [SalScrollAnimationMixin]
};
function _sfc_ssrRender$3(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_nuxt_link = __nuxt_component_0;
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "services__area pt-110 pb-100 grey-bg-15" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-50 text-center"><span class="tp-section-subtitle-3">List Grid</span><h3 class="tp-section-title-3">List Grid Style</h3></div></div></div><div class="row"><div class="col-xl-6 col-lg-6"><div class="services__item-15 d-sm-flex align-items-start transition-3 mb-30" data-sal="slide-up" data-sal-delay="130" data-sal-duration="1000"><div class="services__icon-15 mr-45"><span><svg width="36" height="45" viewBox="0 0 36 45" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M35 11.5306V32.5917C35 41.0161 32.875 43.1222 24.375 43.1222H11.625C3.125 43.1222 1 41.0161 1 32.5917V11.5306C1 3.10611 3.125 1 11.625 1H24.375C32.875 1 35 3.10611 35 11.5306Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M22.2656 8.37244H13.7656" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M17.993 37.0134C19.8121 37.0134 21.2867 35.5519 21.2867 33.749C21.2867 31.9461 19.8121 30.4845 17.993 30.4845C16.1739 30.4845 14.6992 31.9461 14.6992 33.749C14.6992 35.5519 16.1739 37.0134 17.993 37.0134Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="services__content-15"><h3 class="services__title-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`Responsive Design`);
      } else {
        return [
          createTextVNode("Responsive Design")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</h3><p>There are many variations of passages of <br> lorem Ipsum available.!</p><div class="services__btn-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(` Learn More <span${_scopeId}><svg width="32" height="10" viewBox="0 0 32 10" fill="none" xmlns="http://www.w3.org/2000/svg"${_scopeId}><path d="M26.8667 1L31 5.00003L26.8667 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path><path d="M1 4.99878H31" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path></svg></span>`);
      } else {
        return [
          createTextVNode(" Learn More "),
          createVNode("span", null, [
            (openBlock(), createBlock("svg", {
              width: "32",
              height: "10",
              viewBox: "0 0 32 10",
              fill: "none",
              xmlns: "http://www.w3.org/2000/svg"
            }, [
              createVNode("path", {
                d: "M26.8667 1L31 5.00003L26.8667 9",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              }),
              createVNode("path", {
                d: "M1 4.99878H31",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              })
            ]))
          ])
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div></div></div></div><div class="col-xl-6 col-lg-6"><div class="services__item-15 d-sm-flex align-items-start transition-3 mb-30" data-sal="slide-up" data-sal-delay="130" data-sal-duration="1000"><div class="services__icon-15 mr-45"><span><svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.18913 32.5H34.7881C38.7781 32.5 40.8781 30.4 40.8781 26.41V1H3.07812V26.41C3.09913 30.4 5.19913 32.5 9.18913 32.5Z" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path d="M1 1H43" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path d="M13.6016 43L22.0016 38.8M22.0016 38.8V32.5M22.0016 38.8L30.4016 43" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M12.5469 19.9001L19.1619 14.3771C19.6869 13.9361 20.3799 14.0621 20.7369 14.6501L23.2569 18.8501C23.6139 19.4381 24.3069 19.5431 24.8319 19.1231L31.4469 13.6001" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="services__content-15"><h3 class="services__title-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`SEO Optimized`);
      } else {
        return [
          createTextVNode("SEO Optimized")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</h3><p>There are many variations of passages of <br> lorem Ipsum available.!</p><div class="services__btn-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(` Learn More <span${_scopeId}><svg width="32" height="10" viewBox="0 0 32 10" fill="none" xmlns="http://www.w3.org/2000/svg"${_scopeId}><path d="M26.8667 1L31 5.00003L26.8667 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path><path d="M1 4.99878H31" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path></svg></span>`);
      } else {
        return [
          createTextVNode(" Learn More "),
          createVNode("span", null, [
            (openBlock(), createBlock("svg", {
              width: "32",
              height: "10",
              viewBox: "0 0 32 10",
              fill: "none",
              xmlns: "http://www.w3.org/2000/svg"
            }, [
              createVNode("path", {
                d: "M26.8667 1L31 5.00003L26.8667 9",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              }),
              createVNode("path", {
                d: "M1 4.99878H31",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              })
            ]))
          ])
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div></div></div></div><div class="col-xl-6 col-lg-6"><div class="services__item-15 d-sm-flex align-items-start transition-3 mb-30" data-sal="slide-up" data-sal-delay="160" data-sal-duration="1000"><div class="services__icon-15 mr-45"><span><svg width="44" height="41" viewBox="0 0 44 41" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M40.9633 10.0419L34.5296 35.872C34.0714 37.8002 32.3533 39.1365 30.3678 39.1365H5.28222C2.39948 39.1365 0.337686 36.3109 1.19678 33.5427L9.23407 7.73192C9.78771 5.93737 11.4487 4.69629 13.3196 4.69629H36.8015C38.6151 4.69629 40.1233 5.80358 40.7533 7.33085C41.116 8.15177 41.1924 9.08731 40.9633 10.0419Z" stroke="currentColor" stroke-width="2" stroke-miterlimit="10"></path><path opacity="0.4" d="M29.6445 39.1365H38.77C41.2328 39.1365 43.1609 37.0556 42.9891 34.5929L41.0991 8.59094" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path d="M17.5781 9.31629L19.5636 1.06909" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path d="M30.3672 9.33547L32.1617 1.05005" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M13.7969 20.0457H29.0697" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M11.8906 27.6816H27.1634" stroke="currentColor" stroke-width="2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="services__content-15"><h3 class="services__title-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`Daily Updates`);
      } else {
        return [
          createTextVNode("Daily Updates")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</h3><p>There are many variations of passages of <br> lorem Ipsum available.!</p><div class="services__btn-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(` Learn More <span${_scopeId}><svg width="32" height="10" viewBox="0 0 32 10" fill="none" xmlns="http://www.w3.org/2000/svg"${_scopeId}><path d="M26.8667 1L31 5.00003L26.8667 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path><path d="M1 4.99878H31" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path></svg></span>`);
      } else {
        return [
          createTextVNode(" Learn More "),
          createVNode("span", null, [
            (openBlock(), createBlock("svg", {
              width: "32",
              height: "10",
              viewBox: "0 0 32 10",
              fill: "none",
              xmlns: "http://www.w3.org/2000/svg"
            }, [
              createVNode("path", {
                d: "M26.8667 1L31 5.00003L26.8667 9",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              }),
              createVNode("path", {
                d: "M1 4.99878H31",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              })
            ]))
          ])
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div></div></div></div><div class="col-xl-6 col-lg-6"><div class="services__item-15 d-sm-flex align-items-start transition-3 mb-30" data-sal="slide-up" data-sal-delay="160" data-sal-duration="1000"><div class="services__icon-15 mr-45"><span><svg width="42" height="43" viewBox="0 0 42 43" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M33.9358 30.66L34.7158 36.98C34.9158 38.64 33.1358 39.7999 31.7158 38.9399L23.3358 33.96C22.4158 33.96 21.5158 33.9 20.6358 33.78C22.1158 32.04 22.9958 29.84 22.9958 27.46C22.9958 21.78 18.0758 17.1801 11.9958 17.1801C9.67579 17.1801 7.5358 17.84 5.7558 19C5.6958 18.5 5.67578 18 5.67578 17.48C5.67578 8.37998 13.5758 1 23.3358 1C33.0958 1 40.9958 8.37998 40.9958 17.48C40.9958 22.88 38.2158 27.66 33.9358 30.66Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M23 27.4601C23 29.8401 22.12 32.0402 20.64 33.7802C18.66 36.1802 15.52 37.7201 12 37.7201L6.78 40.8201C5.9 41.3601 4.77999 40.6201 4.89999 39.6001L5.39999 35.6602C2.71999 33.8002 1 30.8201 1 27.4601C1 23.9401 2.88001 20.8401 5.76001 19.0001C7.54001 17.8401 9.68 17.1802 12 17.1802C18.08 17.1802 23 21.7801 23 27.4601Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="services__content-15"><h3 class="services__title-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`Fast Support`);
      } else {
        return [
          createTextVNode("Fast Support")
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</h3><p>There are many variations of passages of <br> lorem Ipsum available.!</p><div class="services__btn-15">`);
  _push(ssrRenderComponent(_component_nuxt_link, { href: "/service-details" }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(` Learn More <span${_scopeId}><svg width="32" height="10" viewBox="0 0 32 10" fill="none" xmlns="http://www.w3.org/2000/svg"${_scopeId}><path d="M26.8667 1L31 5.00003L26.8667 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path><path d="M1 4.99878H31" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"${_scopeId}></path></svg></span>`);
      } else {
        return [
          createTextVNode(" Learn More "),
          createVNode("span", null, [
            (openBlock(), createBlock("svg", {
              width: "32",
              height: "10",
              viewBox: "0 0 32 10",
              fill: "none",
              xmlns: "http://www.w3.org/2000/svg"
            }, [
              createVNode("path", {
                d: "M26.8667 1L31 5.00003L26.8667 9",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              }),
              createVNode("path", {
                d: "M1 4.99878H31",
                stroke: "currentColor",
                "stroke-width": "1.5",
                "stroke-linecap": "round",
                "stroke-linejoin": "round"
              })
            ]))
          ])
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div></div></div></div></div></div></section>`);
}
const _sfc_setup$3 = _sfc_main$3.setup;
_sfc_main$3.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ListGridStyle.vue");
  return _sfc_setup$3 ? _sfc_setup$3(props, ctx) : void 0;
};
const ListGridStyle = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["ssrRender", _sfc_ssrRender$3]]);
const _sfc_main$2 = {
  mixins: [SalScrollAnimationMixin]
};
function _sfc_ssrRender$2(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(`<section${ssrRenderAttrs(mergeProps({ class: "support__area pt-110 pb-110" }, _attrs))}><div class="container"><div class="row"><div class="col-xxl-12"><div class="tp-section-wrapper-3 mb-60 text-center"><span class="tp-section-subtitle-3">Minimal &amp; Clean Design</span><h3 class="tp-section-title-3">Simple Grid Style</h3></div></div></div><div class="row"><div class="col-xxl-12"><div class="support__wrapper-2 d-flex flex-wrap align-items-center justify-content-center"><div class="support__item-2 d-flex align-items-center mb-10" data-sal="slide-up" data-sal-delay="130" data-sal-duration="1000"><div class="support__icon-2 mr-15"><span><svg width="20" height="26" viewBox="0 0 20 26" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.6" d="M10.001 7.77075L8.78035 9.90736C8.50656 10.3783 8.73472 10.7689 9.27088 10.7689H10.7196C11.2672 10.7689 11.484 11.1595 11.2102 11.6304L10.001 13.767" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M5.77981 19.4301V18.0976C3.15605 16.5009 1 13.3878 1 10.0795C1 4.3934 6.19049 -0.0636163 12.054 1.22294C14.6322 1.7973 16.8909 3.52038 18.0659 5.89822C20.4501 10.7228 17.9404 15.8461 14.2557 18.0861V19.4186C14.2557 19.7517 14.3812 20.5214 13.1606 20.5214H6.87495C5.6201 20.5328 5.77981 20.0389 5.77981 19.4301Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.6" d="M6.01172 24.9999C8.62407 24.2533 11.3847 24.2533 13.9971 24.9999" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="support__content-2"><h3 class="support__title-2">Knowledge Base</h3></div></div><div class="support__item-2 d-flex align-items-center mb-10" data-sal="slide-up" data-sal-delay="150" data-sal-duration="1000"><div class="support__icon-2 mr-15"><span><svg width="24" height="20" viewBox="0 0 24 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12.5842 19H5.63147C2.15512 19 1 16.7553 1 14.5V5.5C1 2.12233 2.15512 1 5.63147 1H12.5842C16.0605 1 17.2156 2.12233 17.2156 5.5V14.5C17.2156 17.8777 16.0495 19 12.5842 19Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M20.2732 15.4511L17.2148 13.3668V6.62211L20.2732 4.53779C21.7693 3.52235 23.0014 4.1423 23.0014 5.92734V14.0722C23.0014 15.8573 21.7693 16.4772 20.2732 15.4511Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path opacity="0.4" d="M11.4509 8.93126C12.3623 8.93126 13.1011 8.21343 13.1011 7.32793C13.1011 6.44244 12.3623 5.72461 11.4509 5.72461C10.5396 5.72461 9.80078 6.44244 9.80078 7.32793C9.80078 8.21343 10.5396 8.93126 11.4509 8.93126Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="support__content-2"><h3 class="support__title-2">Video Tutorials</h3></div></div><div class="support__item-2 d-flex align-items-center mb-10" data-sal="slide-up" data-sal-delay="170" data-sal-duration="1000"><div class="support__icon-2 mr-15"><span><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.4" d="M19.1172 17.313L19.5462 20.789C19.6562 21.702 18.6772 22.34 17.8962 21.867L13.2872 19.128C12.7812 19.128 12.2862 19.095 11.8022 19.029C12.6162 18.072 13.1002 16.862 13.1002 15.553C13.1002 12.429 10.3942 9.89903 7.05022 9.89903C5.77422 9.89903 4.59723 10.262 3.61823 10.9C3.58523 10.625 3.57422 10.35 3.57422 10.064C3.57422 5.05899 7.91922 1 13.2872 1C18.6552 1 23.0002 5.05899 23.0002 10.064C23.0002 13.034 21.4712 15.663 19.1172 17.313Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M13.1 15.5531C13.1 16.8621 12.616 18.0722 11.802 19.0292C10.713 20.3492 8.986 21.1961 7.05 21.1961L4.179 22.9011C3.695 23.1981 3.079 22.7911 3.145 22.2301L3.42 20.0632C1.946 19.0402 1 17.4011 1 15.5531C1 13.6171 2.03401 11.9121 3.61801 10.9001C4.59701 10.2621 5.774 9.89917 7.05 9.89917C10.394 9.89917 13.1 12.4291 13.1 15.5531Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span></div><div class="support__content-2"><h3 class="support__title-2">Support Forum</h3></div></div></div></div></div></div></section>`);
}
const _sfc_setup$2 = _sfc_main$2.setup;
_sfc_main$2.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/SimpleGridStyle.vue");
  return _sfc_setup$2 ? _sfc_setup$2(props, ctx) : void 0;
};
const SimpleGridStyle = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["ssrRender", _sfc_ssrRender$2]]);
const _sfc_main$1 = {
  components: {
    CommonBreadcrumb: TeamBreadcrumb,
    HomeServices,
    ElBgImgStyle,
    FreelancerAwards,
    PersonalPortServices,
    ElIconBoxStyleFour,
    ElBgClrBox,
    ElClrIconBox,
    ElListIconBox,
    ElIconBoxStyleThree,
    StartUpCounter,
    ShopFeatures,
    ElIconBoxStyleFive,
    ElAnimatedBorderStyle,
    ElIconBoxStyleSix,
    ElIconBoxStyleSeven,
    BoxShadowStyle,
    SimpleGridStyle,
    BoxShadowWithBorder,
    ListGridStyle,
    ContactIconBox,
    ContactIconBoxTwo
  }
};
function _sfc_ssrRender$1(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_common_breadcrumb = resolveComponent("common-breadcrumb");
  const _component_home_services = resolveComponent("home-services");
  const _component_el_bg_img_style = resolveComponent("el-bg-img-style");
  const _component_freelancer_awards = resolveComponent("freelancer-awards");
  const _component_personal_port_services = resolveComponent("personal-port-services");
  const _component_el_bg_clr_box = resolveComponent("el-bg-clr-box");
  const _component_el_clr_icon_box = resolveComponent("el-clr-icon-box");
  const _component_el_list_icon_box = resolveComponent("el-list-icon-box");
  const _component_el_icon_box_style_three = resolveComponent("el-icon-box-style-three");
  const _component_start_up_counter = resolveComponent("start-up-counter");
  const _component_el_icon_box_style_four = resolveComponent("el-icon-box-style-four");
  const _component_shop_features = ShopFeatures;
  const _component_el_icon_box_style_five = resolveComponent("el-icon-box-style-five");
  const _component_el_animated_border_style = resolveComponent("el-animated-border-style");
  const _component_el_icon_box_style_six = resolveComponent("el-icon-box-style-six");
  const _component_el_icon_box_style_seven = resolveComponent("el-icon-box-style-seven");
  const _component_box_shadow_style = resolveComponent("box-shadow-style");
  const _component_simple_grid_style = resolveComponent("simple-grid-style");
  const _component_box_shadow_with_border = resolveComponent("box-shadow-with-border");
  const _component_list_grid_style = resolveComponent("list-grid-style");
  const _component_contact_icon_box = resolveComponent("contact-icon-box");
  const _component_contact_icon_box_two = resolveComponent("contact-icon-box-two");
  _push(`<!--[-->`);
  _push(ssrRenderComponent(_component_common_breadcrumb, {
    title: "Have a look to our Icon Box",
    subtitle: "Elements"
  }, null, _parent));
  _push(ssrRenderComponent(_component_home_services, { el_style: true }, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_el_bg_img_style, null, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_freelancer_awards, { el_style: true }, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_personal_port_services, { el_style: true }, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_el_bg_clr_box, null, null, _parent));
  _push(ssrRenderComponent(_component_el_clr_icon_box, null, null, _parent));
  _push(ssrRenderComponent(_component_el_list_icon_box, null, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_el_icon_box_style_three, null, null, _parent));
  _push(ssrRenderComponent(_component_start_up_counter, { el_style: true }, null, _parent));
  _push(ssrRenderComponent(_component_el_icon_box_style_four, null, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_shop_features, null, null, _parent));
  _push(ssrRenderComponent(_component_el_icon_box_style_five, null, null, _parent));
  _push(ssrRenderComponent(_component_el_animated_border_style, null, null, _parent));
  _push(ssrRenderComponent(_component_el_icon_box_style_six, null, null, _parent));
  _push(ssrRenderComponent(_component_el_icon_box_style_seven, null, null, _parent));
  _push(ssrRenderComponent(_component_box_shadow_style, null, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_simple_grid_style, null, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_box_shadow_with_border, null, null, _parent));
  _push(ssrRenderComponent(_component_list_grid_style, null, null, _parent));
  _push(ssrRenderComponent(_component_contact_icon_box, null, null, _parent));
  _push(`<div class="tp-separator"></div>`);
  _push(ssrRenderComponent(_component_contact_icon_box_two, null, null, _parent));
  _push(`<!--]-->`);
}
const _sfc_setup$1 = _sfc_main$1.setup;
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("elements/elements-iconbox/ElementIconBoxMain.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const ElementIconBoxMain = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["ssrRender", _sfc_ssrRender$1]]);
const _sfc_main = {
  components: {
    HeaderOne,
    ElementIconBoxMain,
    Footer,
    BackToTop
  },
  setup() {
    useHead({
      title: "Element IconBox - Creative Agency & Portfolio Vue Nuxt 3 Template"
    });
  }
};
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  const _component_header_one = resolveComponent("header-one");
  const _component_element_icon_box_main = resolveComponent("element-icon-box-main");
  const _component_Footer = resolveComponent("Footer");
  const _component_back_to_top = resolveComponent("back-to-top");
  _push(`<div${ssrRenderAttrs(_attrs)}>`);
  _push(ssrRenderComponent(_component_header_one, {
    top_bar: false,
    commonOffcanvas: true
  }, null, _parent));
  _push(ssrRenderComponent(_component_element_icon_box_main, null, null, _parent));
  _push(ssrRenderComponent(_component_Footer, null, null, _parent));
  _push(ssrRenderComponent(_component_back_to_top, null, null, _parent));
  _push(`</div>`);
}
const _sfc_setup = _sfc_main.setup;
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/elements-iconbox.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const elementsIconbox = /* @__PURE__ */ _export_sfc(_sfc_main, [["ssrRender", _sfc_ssrRender]]);
export {
  elementsIconbox as default
};
//# sourceMappingURL=elements-iconbox-DhShjNiG.js.map
